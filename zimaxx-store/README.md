...
> **Desde 2026-09-14** ese error viaja además con `code: 'customer_no_address'`
> y la bandeja de Pedidos, en vez de mandar a la vendedora a SellerCloud,
> ofrece **📍 Cargar dirección y reenviar** (guarda la dirección en el cliente,
> la carga en SellerCloud vía `sellercloud-customers` y reenvía la orden — ver
> la fila Pedidos de la sección 2 y "Dirección del cliente" en la sección 3).
...
#### El banner es un cuadro expandible que dice qué había en cada pedido (2026-09-10)

Al correr la migración de descartar, el banner mostró de golpe 77 fallos
acumulados desde el 08-12 y ocupaba media pantalla sin decir qué había
adentro. Ahora arranca **plegado**, con el conteo total y por tipo ("3 Pedidos
· 1 Cotización") y un botón **"Ver detalle ▾"**. Dentro, cada fallo lleva el
badge **Pedido / Cotización** (lo que el cliente intentó, `order_failures.kind`)
junto al nombre, y un botón **"Ver contenido ▾"** que despliega sus líneas:
Producto · Cantidad, más Precio unit. · Subtotal · Total solo si el payload
traía precios (el del outbox viaja slim, `{id, qty}`, y los nombres se resuelven
desde `products`). Las líneas que nombra el motivo "productos sin precio en la
lista del cliente" van **en rojo con etiqueta**, así se ve de un vistazo cuál
fue la que hizo rebotar el pedido entero. Un producto borrado dice "Producto no
encontrado"; un fallo por token inválido no tiene ítems y solo ofrece Descartar.

**Desde 2026-09-11, cada fallo dice de cuándo es y todos se pueden descartar.**
La fecha ya estaba, pero en gris chico delante del motivo y con el formato del
navegador; ahora va en **su propia línea**, en el idioma del panel y con el
día relativo ("📅 mar, 8 sept. 2026, 03:00 p. m. · hace 3 días" / "ayer" /
"hoy"), y el resumen plegado dice el rango ("📅 del 12 ago. 2026 al 11 sept.
2026") para saber de cuándo son sin abrir la lista. Y **"Descartar" aparece
en todos los fallos**, también en los recuperables, por si no se quiere
recuperar: en esos abre un modal de confirmación que dice de quién y de cuándo
es el intento y avisa que desde el panel no se deshace (la fila no se borra y
queda en la auditoría); sin cliente o sin ítems sigue siendo directo. El
tope del cuadro subió de 50 a 200 filas. Ojo: para un fallo del outbox
expirado la fecha es cuando el teléfono lo reportó (≥24 h después), no cuando
el cliente tocó Enviar — la RPC no viaja con esa hora.

Diagnóstico de esos 77 (2026-09-10): no era una función apagada — 75 eran
rechazos por "sin precio en la lista" (regla de 2026-08-06), porque el carrito
persiste en el teléfono y conserva productos que la carga semanal de precios
sacó de la lista. De 145 líneas-SKU, 64 ya tienen precio hoy (se recuperan
como cotización y se convierten) y 81 siguen sin precio (cargar el precio o
descartar).
...
Apareció entonces un botón **"Descartar"** exactamente en esos casos (desde
2026-09-11 está en todos los fallos, con confirmación en los recuperables —
ver arriba): no borra la fila, solo la saca del banner
(`order_failures.dismissed_at`) y queda auditada. Requiere correr `migration-2026-08-13-recover-as-quote.sql` y
`migration-2026-08-13-dismiss-order-failures.sql` — la segunda **junto con el
deploy**, porque el frontend ya filtra por la columna nueva. **Estado
(2026-09-10)**: las dos están corridas en producción; la de descartar la
corrió el usuario ese día, y hasta entonces —con el frontend ya desplegado—
...
SellerCloud desde 2026-08-31): convierte la celda Nombre en tres inputs
apilados — nombre, teléfono y correo (desde 2026-09-10 el teléfono vive
debajo del nombre en esa misma celda; antes tenía columna propia) —
...
vincula igual y lo que no entró vuelve como aviso. **Desde 2026-09-11** ese
mismo PUT lleva además `IsWholesale: true` y `CompanyId` (la compañía del
negocio, `SELLERCLOUD_COMPANY_ID`, la misma de las órdenes) — explícito a
pedido del usuario, porque el enum `CustomerType` del create es ambiguo en el
Swagger y así el customer queda mayorista y en la compañía correcta sin
depender de él; también aplica al botón de actualizar ficha. El campo
Empresa del formulario ya no dice "(opcional)" en el placeholder.
...
vacío"). El `POST /Customers` de SellerCloud **no acepta dirección** — se
carga aparte con `PUT /Customers/{id}/Addresses` — y hasta hoy no se mandaba
ni se guardaba en ningún lado; como el envío de órdenes lee la dirección del
customer allá y rebota si no hay ninguna, 6 de los 9 pushes fallidos del 07
al 14 de septiembre fueron por eso (dos de esos clientes se habían creado
desde este catálogo). Ahora cada cliente tiene **una** dirección
(`migration-2026-09-14-client-address.sql`: `address_line1`, `address_line2`,
`address_city`, `address_state`, `address_zip`, `address_country` en ISO-2,
más `sc_address_id`, `sc_address_synced_at` y `sc_address_error` para saber
si está en SellerCloud y verificada), que allá vale como envío **y**
facturación. Decisiones del usuario: **estado obligatorio para todos los
países** (en Venezuela también existe: Miranda, Zulia…); **obligatoria solo
cuando el cliente se crea en SellerCloud** — con el toggle "Crear también en
SellerCloud" prendido los campos son `required`; apagado, puede quedar vacía
pero **nunca a medias** (el mensaje lista exactamente lo que falta; la RPC y
la Edge Function repiten la regla, así que un bundle viejo tampoco la
saltea). El país se propone por la lista (`ve_*` → Venezuela, el resto →
Estados Unidos, cambiable) y el campo Estado tiene sugerencias para US
(código de dos letras: FL) y Venezuela. **Al crear** el customer, la Edge
Function exige la dirección completa en la fila (sin ella responde 400 y no
crea nada), crea el customer, manda la ficha, vincula y después carga la
dirección con **GET → PUT → GET**: manda la lista completa del customer con la
nuestra adelante (el Swagger no dice si el PUT reemplaza o mezcla), relee y
la busca por ID o por calle + código postal — si no aparece es error, porque
la orden rebotaría igual — y la anota con la RPC `mark_client_sc_address`. Ese
resultado viaja **aparte** del ✓ del create: si la dirección no entró, el
panel muestra un cuadro rojo con **Reintentar dirección** — el customer existe
y está vinculado, pero no puede recibir órdenes hasta que entre. **Edición**:
el bloque aparece en la misma fila desplegada que la ficha; Guardar pasa por
la RPC auditada `update_client_address` (admin cualquiera, vendedora sus
...
**Dirección** y **Dir. SellerCloud** (14 en total). El sync de n8n
(`sync_upsert_clients`) acepta la dirección por fila de forma opcional y, si
viene utilizable, la guarda **como sincronizada**: para un customer que ya
existe allá, SellerCloud es la fuente de verdad (contrato en la sección 7).
...
(5 columnas: Nombre, SellerCloud, Lista, Vendedora y acciones); la completa
muestra **todas las columnas registradas** — Nombre, Empresa, Email,
SellerCloud, Grupo, Account manager (nombre + #id), Salesman, Comentarios,
Lista, Vendedora y Alta (12). En la completa el correo tiene columna propia
(la flechita ✉️ es solo de la compacta). El buscador también matchea empresa,
grupo, salesman y comentarios ("zimaxx box" trae esos clientes).

**El teléfono va debajo del nombre** (2026-09-10, a pedido del usuario:
"quita la columna de teléfono y haz que salga debajo del nombre"): la columna
Tel desapareció de las dos vistas; el teléfono se muestra en mono chico bajo
el nombre, con la flechita ✉️ del correo pegada (solo en la compacta), y al
editar los tres inputs —nombre, teléfono, correo— van apilados en esa misma
celda. La tabla compacta pasó de 6 a 5 columnas y la completa de 13 a 12.

**Cómo se desplaza a los lados** (mismo día, segunda iteración — el usuario
reportó que "no había manera de scrollear a los lados"): con 12 columnas la
...
- **Métricas** (2026-08-06, `migration-2026-08-06-sa-metrics.sql`): una sola
  ...
  - **Histórico** (2026-09-11, `migration-2026-09-11-sa-metrics-historic.sql`):
    `p_days = 0` = desde las 00:00 del primer pedido real (sin cuentas de
    prueba) hasta ahora, sin clamp; `period` gana `historic` / `since` /
    `bucket`, y con más de 120 días la serie sale por semana (`bucket =
    'week'`, `dia` = lunes). `p_days > 0` sigue igual (clamp `[1, 365]`; un
    valor a mano mayor a 120 también sale semanal). Probada en un PostgreSQL
    18 desechable: 8 escenarios (histórico con cuenta de prueba más vieja que
    el primer pedido real, 14 días intactos, clamps, `not authorized`, base
    vacía, grants, idempotencia).
...
> cliente en `clients` (+ estado de su sync con SellerCloud), RPC
> `update_client_address` y `mark_client_sc_address`, `sync_upsert_clients`
> acepta la dirección por fila (opcional: el payload actual de n8n sigue
> corriendo igual) y el helper `sync_country_iso2`. Aditiva e idempotente, con
> preflight; probada ×2 + asserts + preflight negativo en PG 18 desechable.
> **Orden**: migración → `npx supabase functions deploy sellercloud-customers`
> (→ v8) y `npx supabase functions deploy sellercloud-push-order` (→ v15),
> desde `zimaxx-store/` → deploy del frontend (commit en `dev`, `main`
> fast-forward, push). El frontend nuevo sin la migración falla al insertar
> un cliente con dirección (42703); la función nueva sin ella responde "falta
> la dirección" al crear (correcto: sin la migración no hay dónde cargarla).
> **Antes de redesplegar desde `dev`, los dos flags del alta en `false`** y
> volver a `true` después (regla de la fila Clientes de la sección 2). Para
> n8n: agregar `address/address2/city/state/zip/country/sc_address_id` por
> fila (de `Addresses[]` del customer, la marcada `IsShippingAddress` o la
> única) para que los customers existentes lleguen con su dirección; sin esas
> claves nada cambia. Contrato completo al pie de la migración.
...
- **✅ `migration-2026-08-06-require-price.sql` corrida** (confirmado por el
  ...
  - Probada contra un PostgreSQL 18 desechable: primero se **reprodujo el agujero
    ...
    encima.
- **HECHO: `migration-2026-09-11-sa-metrics-historic.sql` está corrida**
  (2026-09-11, el usuario con `npx supabase db query --linked -f`; verificado en
  `pg_proc`). Rango "Histórico" en 📈 Métricas: reemplaza `sa_metrics_overview`
  con la misma firma. Falta solo el deploy del frontend con el botón.
