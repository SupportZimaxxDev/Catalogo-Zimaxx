...
> `migration-2026-09-15-price-list-min-order.sql` SIN CORRER y frontend SIN
> desplegar: **frontend primero, migración después**. Impacto medido: la mitad
> de los pedidos de `us_wholesale` hoy está por debajo de $2,000).
...
| `migration-2026-09-15-price-list-min-order.sql` | ⏳ **SIN CORRER** (2026-09-15, punto 90; probada ×2 sobre el estado vivo reconstruido en PG 18 desechable + 12 bloques de assert; `get_catalog`/`create_order` vivos verificados idénticos al repo con `pg_get_functiondef` antes de copiarlos) | `price_lists.min_order` (semilla 800 / 2000 / null solo la primera vez, default 800, check), `get_catalog` con `client.min_order`, `create_order` rechazando por debajo del mínimo (`order_failures`). **Orden: frontend PRIMERO** (sin la clave cae al 800 de siempre), migración después; al revés, el carrito viejo deja mandar pedidos mayoristas de $800–$1,999 que el servidor rechaza. Sondeo: `position('pedido mínimo' in pg_get_functiondef('public.create_order(text, jsonb, numeric, text, uuid)'::regprocedure)) > 0` |
