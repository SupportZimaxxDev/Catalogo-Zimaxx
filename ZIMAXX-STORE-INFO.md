...
> README.
>
> **✅ ESTADO DE MIGRACIONES (2026-09-10, sondeado EN PRODUCCIÓN con
> `supabase db query --linked`): no queda ninguna pendiente.** La última era
> `migration-2026-08-13-dismiss-order-failures.sql` (punto 60), que el usuario
> corrió el 2026-09-10 (`order_failures.dismissed_at` y `dismiss_order_failure`
> existen). Hasta ese día el frontend desplegado, que ya filtraba por
> `dismissed_at`, recibía `42703` en `loadFailures()` y — como el `.then`
> descarta el error — **el banner rojo de Pedidos se veía VACÍO en silencio,
> con 77 fallos sin recuperar detrás**: no "rompía la pestaña", la escondía.
> Todas las demás, incluidas 08-24, 08-26, 09-02 ×3, 09-04 y 09-09, están
> corridas (verificado contra `pg_proc`/`information_schema`; los "pendiente"
> del checklist de abajo son históricos). Ver el punto 82 para el resto del
> estado real de producción (funciones vivas, `superadmin-users` redesplegada
> ese día, una policy en forma vieja).
>
...
> `recover-as-quote` (`recover_order_failure` ya crea cotización). **CORRECCIÓN
> 2026-09-17: lo de `recover-as-quote` era FALSO.** El preflight de la 09-17
> rechazó el `recover_order_failure` vivo y `pg_get_functiondef` (vía
> `supabase db query --linked`) mostró la versión del 08-05 (`v_kind := case
> when v_fail.kind = 'quote' ...`): la 08-13 NUNCA corrió, y las 89
> recuperaciones hechas entre el 08-07 y el 09-17 crearon pedidos REALES con
> precio congelado (34 atendidos, 50 cancelados, 5 nuevos), no cotizaciones.
> La absorbe la 09-17 (punto 91), que trae el "siempre cotización"; la 08-13
> quedó con un guard y ya no se corre. También
...
>
> **✅ 2026-09-10: `superadmin-users` REDESPLEGADA (v2)** con
> `npx supabase functions deploy superadmin-users` desde `zimaxx-store/`, a
> pedido del usuario; sonda sin JWT → `401` (viva), igual que
> `admin-create-vendedora-user`. El hallazgo de abajo estuvo vigente desde
> antes del 08-19 hasta ese día: durante ese tiempo "Cambiar contraseña" y
> "+ Crear admin" del panel 🔐 estaban rotos. Ver punto 82.
>
...
> recuperación". `order_failures` suma `recovered_at`, `recovered_by`,
> `recovered_by_email` y `recovery_seen_at` (null = aviso pendiente; backfill
> de las históricas, que nacen vistas); `recover_order_failure` las llena — si
> la recupera la propia dueña o el cliente no tiene vendedora nace visto (sin
> autoaviso) — y pasa a compare-and-set; RPC nueva `mark_recoveries_seen(uuid[])`.
> Frontend: campanita 🔔 en el header **solo para vendedoras** (contador,
> "Se recuperó el pedido / la cotización de X", cuándo y por quién, **Ver** →
> la cotización en Pedidos, **Marcar visto** explícito por fila o todos; poll
> 60 s + al volver a la pestaña) y chip ♻️ Recuperado en la fila de la bandeja
> (tooltip cuándo/quién). **En Pedidos nada más**: se construyó un cuadro
> verde "♻️ Pedidos recuperados" con el historial y el usuario pidió quitarlo
> el mismo día ("el registro quede nada más en el registro de movimientos… el
> panel verde quitalo de ahí, el rojo… ese sí lo puedes dejar"). **Y todo
> identificado en el Registro de movimientos** (segunda vuelta, a pedido del
> usuario): la recuperación etiquetada por cómo entró ("Pedido recuperado
> como cotización" / "Cotización recuperada" / "Pedido recuperado como pedido
> real" para las 89 viejas) con intento original, motivo y aviso pendiente;
> el visto como movimiento propio `recovery_seen` ("Aviso de recuperación
> visto"); y el descarte con etiqueta ("Intento descartado", antes caía en
> reasignación). Decisiones
> del usuario: campanita (no badge en la pestaña), visto explícito (no al
> abrir), sin autoaviso, todo en el Registro de movimientos y nada de
> historial en Pedidos. Migración
> `migration-2026-09-17-recovery-notifications.sql` **SIN correr en
> producción** — el primer intento abortó en el preflight y destapó que el
> `recover_order_failure` vivo era el del 08-05: la 08-13 "siempre
> cotización" NUNCA corrió (89 recuperaciones crearon pedidos reales: 34
> atendidos, 50 cancelados, 5 nuevos); la 09-17 la absorbe y acepta las dos
> versiones — y frontend **SIN desplegar**; orden: migración primero
> (aditiva; el frontend nuevo sin ella degrada por 42703 a "sin campanita ni
> chips ♻️"). Probada ×2 + 11 bloques de assert + 2 preflights negativos
> en PG 18 desechable, 49/49 Playwright contra el build).
> Anterior: 2026-09-15 (**pedido mínimo por
> lista de precio**, punto 90, a pedido del usuario: "sería bueno limitar la
> creación de órdenes menores a 2000 al catálogo de 800$" — `price_lists.min_order`
> 800 / 2,000 / null, `get_catalog` con `client.min_order`, `create_order`
> rechazando por debajo del mínimo; migración
> `migration-2026-09-15-price-list-min-order.sql` **CORRIDA el mismo día por
> el usuario** (verificada en `pg_proc`/`price_lists`) y frontend **SIN
> desplegar** — corrió al revés del orden recomendado, así que hasta el deploy
> el carrito viejo deja mandar pedidos mayoristas de $800–$1,999 que el
> servidor rechaza a `order_failures`. Impacto medido: la mitad de los
> pedidos de `us_wholesale` hoy está por debajo de $2,000).
> Anterior: 2026-09-14, tercera tanda
> (**dirección del cliente para SellerCloud**, punto 89, a pedido del usuario:
> "es muy importante que el cliente se guarde con el address correcto, y que
> ese campo no llegue vacío"). El `POST /Customers` no acepta dirección — va
> aparte con `PUT /Customers/{id}/Addresses` — y hasta hoy no se mandaba ni se
> guardaba; el push de órdenes rebota si el customer no tiene ninguna (6 de
> los 9 fallos de septiembre). Ahora `clients` tiene UNA dirección (envío =
> facturación, estado obligatorio para todos los países, obligatoria solo
> para crear el customer allá) + el estado de su sync
> (`migration-2026-09-14-client-address.sql`: RPC `update_client_address` y
> `mark_client_sc_address`, `sync_upsert_clients` acepta la dirección por
> fila); `sellercloud-customers` la exige para `create` y la carga con GET →
> PUT → GET verificando por relectura, también en `update`; Clientes tiene el
> bloque 📍 Dirección en alta/edición/panel y un badge por fila con
> Enviar/Reintentar; Pedidos ofrece "📍 Cargar dirección y reenviar" cuando el
> push rebota por eso. **La sesión se cortó por una falla eléctrica** con el
> código completo pero sin verificar ni documentar; se retomó desde el
> scratchpad de esa sesión. Verificado: migración ×2 + 8 bloques de assert +
> preflight negativo en PG 18 desechable, `sc-customers-tests` 53 → **78**,
> i18n **621/621**, **78/78 Playwright** contra el build real, build limpio.
> Ajuste posterior del mismo día (usuario): la fila con la dirección ya
> verificada muestra **✓ + la dirección abreviada** bajo el ID de SellerCloud,
> no un texto fijo "Dirección en SellerCloud".
> **Pendiente (usuario)**: correr la migración, redesplegar
> `sellercloud-customers` (v8) y `sellercloud-push-order` (v15) — con los dos
> flags del alta en `false` antes, como manda la regla del punto 88 — y el
> deploy del frontend; n8n puede sumar la dirección al payload del sync.
>
> Antes: 2026-09-14, primera y segunda tanda (**preparación del
> deploy de todo lo pendiente del 09-08 al 09-11 + el alta en SellerCloud
> vuelve a quedar bloqueada**, punto 88, a pedido del usuario: "quiero
> desplegar los cambios que tenemos hechos por ciertas correcciones pero
> también quiero volver a bloquear la función de creación de clientes a
> SellerCloud"; a mitad de la tanda: "no hagas el despliegue, de eso me
> encargo yo"). Los dos flags (`SC_CREATE_ENABLED` en `ClientsAdmin.jsx` y
> `CREATE_ENABLED` en `sellercloud-customers`) pasan a `false` **también en
> `dev`** — se deroga el "valor por rama" del 09-10. **Hecho**:
> `sellercloud-customers` **redesplegada → v7** (bajada y diffeada: idéntica
> al repo, candado cerrado; la v6 del 09-11 lo tenía abierto) — ese redeploy
> corrió antes de que llegara el pedido de no desplegar. **Lo hace el
> usuario**: commit en `dev`, push, `main` fast-forward a `dev` y push →
> Netlify construye `main` (puntos 78-87: Excel de lista, ✉️ correo, ficha
> SellerCloud + vista completa, Sistema export, teléfono bajo el nombre,
> cuadro de fallos con fecha/contenido/Descartar, mayorista+compañía,
> Histórico en Métricas). **Nada está commiteado**: todo el árbol de trabajo
> de `dev` (features + flags + docs + la migración 09-11) está listo para ese
> commit. Verificado: build limpio, sc-customers 53/53, i18n 583/583, **35/35
> Playwright** del candado apagado contra el build real (`hotfix.mjs` del
> 09-10), chunk de Clientes con `scCreateDisabled` ×4 y sin
> `businessNameOptional`. **Segunda tanda del día**: el usuario hizo el deploy
> (`7924120` = `main` = `dev`; el bundle de Netlify sirve `index-BmDRxyVU.js`,
> el mismo hash que el build local, con `scCreateDisabled` ×4 y los marcadores
> de los puntos 84-87: producción tiene los puntos 78-87 vivos y el alta
> bloqueada en los dos lados) y pidió "y ahora sigamos trabajando el dev con
> la función en true": los dos flags vuelven a `true` en el árbol de trabajo
> de `dev` (sin commitear), con el comentario de VALOR POR RAMA (`main` false,
> `dev` true) y la regla de ponerlos en `false` antes de cada deploy desde
> `dev` — `functions deploy` sube el árbol de trabajo, no la rama. Verificado:
> build limpio + **25/25 Playwright** del flujo habilitado (`flag-on.mjs` del
> 09-10).
>
> Antes: 2026-09-11, tercera tanda
> (**rango "Histórico" en 📈 Métricas**, punto 87, a pedido del usuario: "que
> el rango mantenga las opciones que ya se tienen y que además se agregue un
> histórico, desde que se hizo la primera orden hasta la actual"). Nueva
> `migration-2026-09-11-sa-metrics-historic.sql` (misma firma de
> `sa_metrics_overview`; `p_days = 0` = histórico; serie semanal en rangos
> largos), botón "Histórico" junto a 7/14/30, nota del gráfico con la fecha del
> primer pedido, y detección de RPC vieja. Probada en PostgreSQL 18 desechable
> (8 escenarios) + 30/30 Playwright + build limpio + i18n 583/583. **Migración
> corrida en producción el mismo día por el usuario** (`db query --linked -f`;
> verificado en `pg_proc`: la función viva tiene `v_historic` y el bucket
> semanal). Hoy el histórico real arranca el 2026-07-27 (47 días, 718 pedidos),
> así que sale por día. **Pendiente: deploy del frontend.**
>
> Antes, mismo día, segunda tanda
> (**correcciones al alta de clientes en SellerCloud**, punto 86, a pedido del
> usuario): el PUT de la ficha lleva `IsWholesale: true` + `CompanyId` del
> negocio (también en 'update'), y el placeholder de Empresa ya no dice
> "(opcional)". Frontend + Edge Function, sin migración. sc-customers 53/53,
> build limpio, i18n 578/578. **`sellercloud-customers` desplegada por el
> usuario el mismo día → v6** (sonda sin JWT 401 = viva), con
> `CREATE_ENABLED = true`: **el candado del servidor quedó ABIERTO**; producción
> sigue protegida solo por el flag del frontend de `main`
> (`SC_CREATE_ENABLED = false`). **Pendiente: deploy del frontend** — al hacerlo
> desde `dev` (flag `true`) el alta en SellerCloud vuelve a estar activa para
> todas las vendedoras. (**2026-09-14**, punto 88: los dos flags volvieron a
> `false` en `dev` y la función se redesplegó → v7 con el candado cerrado; el
> deploy del frontend con ese valor lo hace el usuario.)
>
> Antes, mismo día (**fecha visible por
> fallo en "Pedidos que no se registraron" + "Descartar" en todos los fallos,
> con confirmación en los recuperables**, punto 85, a pedido del usuario: "que
> tengan la fecha de cuando son, además de la opción de descartarlos por si no
> se quiere recuperar"). Solo frontend, sin migración — `dismiss_order_failure`
> existe desde la 08-13 y ya acepta cualquier fallo; el tope del cuadro sube de
> 50 a 200 filas. Build limpio; i18n 579/579; **40/40 Playwright**. **Pendiente:
> deploy del frontend** (junto con lo del 09-10).
>
> Antes, 2026-09-10, tercera tanda (**el
> banner de "Pedidos que no se registraron" pasa a cuadro expandible, con
> badge Pedido/Cotización y "Ver contenido" por fallo**, punto 84, a pedido
> del usuario). Diagnóstico de los 77 fallos que aparecieron de golpe: NO son
> funciones apagadas, son rechazos por "productos sin precio en la lista del
> cliente" (regla del punto 52) acumulados desde el 08-12 y mudos hasta la
> migración de hoy; 64 de las 145 líneas-SKU ya tienen precio hoy, 81 no. Sin
> migración; build limpio; 28/28 Playwright. **Pendiente: deploy del frontend.**
>
> Antes, mismo día, segunda tanda (**el
> teléfono va debajo del nombre en la tabla de Clientes**, punto 83, a pedido
> del usuario: "quita la columna de teléfono y haz que salga debajo del
> nombre"). Solo frontend, sin migración: la columna Tel desaparece de las dos
> vistas, el teléfono sale en mono chico bajo el nombre con la flechita ✉️
> pegada, al editar los tres inputs (nombre, teléfono, correo) van apilados en
> esa celda, y `colCount` pasa de 6/13 a 5/12. Verificado: build limpio +
> 25/25 aserciones Playwright (compacta, completa y móvil 390 px). **Pendiente:
> deploy del frontend** (el mismo de siempre).
>
> Antes, mismo día (**auditoría doc ↔
> código ↔ producción + hotfix re-aplicado en `dev`**, punto 82, a pedido del
> usuario). El merge `main` → `dev` del 09-09 (`7850597`) había resuelto los
> conflictos de `ClientsAdmin.jsx` y de este archivo con la versión de `dev`,
> así que en `dev` el candado del alta en SellerCloud (`SC_CREATE_ENABLED`,
> hotfix `bf08618`) y su documentación se habían perdido — el toggle nacía
> prendido y el panel ofrecía "Crear en SellerCloud" contra una Edge Function
> que ya respondía 503. Hoy se re-aplicó el bloque completo del flag (6 hunks)
> sobre el WIP de la ficha, que queda intacto, y **a pedido del usuario los
> dos flags quedaron en `true` en `dev`** ("en esta versión de dev estamos
> reparando la herramienta de crear clientes en SellerCloud, aquí no puede
> estar bloqueada la función"): el mecanismo existe en las dos ramas y solo
> cambia el valor — **`main`/producción `false`, `dev` `true`**. Verificado:
> build limpio + Playwright contra el build real con Supabase y la Edge
> Function interceptadas en los DOS estados — **35/35** con el flag en `false`
> (toggle apagado y deshabilitado con aviso, un solo campo Nombre, alta local
> con CERO llamadas a la función, panel solo buscar/vincular en es/en,
> vendedora con el mismo candado) y **25/25** con el flag en `true`, el estado
> final de `dev` (toggle prendido, nombre partido, el alta llama `create` con
> first/last, panel con "Crear en SellerCloud" y "Vincular", ficha guardada
> por RPC antes del create). Producción verificada el mismo día: frontend =
> `main`; `sellercloud-customers` v4 = hotfix SIN la ficha (el redeploy del
> WIP quedó pisado) → a las 14:29 el usuario la redesplegó desde el árbol de
> trabajo: **v5 = candado `false` + ficha + `update`**; `superadmin-users`
> estaba sin desplegar y **se redesplegó (v2)**; `dismiss-order-failures`
> corrida hoy por el usuario. **OJO al deployar desde
> `dev`** (frontend o función): con los flags en `true` el alta se reactiva en
> producción para todas las vendedoras aunque la reparación no haya terminado
> — confirmar el valor antes de cada deploy.
>
> Antes: 2026-09-09, tercera tanda
> (**⚙️ Sistema: filtro "clientes creados desde el catálogo" + búsqueda +
> Exportar Excel**, punto 81, a pedido del usuario). Sin migración: cada alta
> de cliente (formulario o Excel) deja un `client_created` en `system_logs`
> (source nuevo `clients`, context = ficha por nombres + `created_by`), la
> pestaña gana un chip que filtra por ese origen, búsqueda de texto sobre lo
> cargado y un Excel que recorre TODAS las páginas de `get_system_logs` (tope
> 5,000) con el `context` aplanado en columnas. Verificado: 23 Node + 62
> Playwright nuevas, Clientes 144/144, build limpio. **Pendiente: deploy del
> frontend** (el mismo que ya estaba pendiente).
>
> Antes, mismo día (**ficha SellerCloud
> del cliente + vista completa de Clientes**, punto 80, a pedido del usuario:
> "el cliente se debe crear con business name, customer group, account
> manager, salesmen y comments" y "un botón de expandir que cambie a una vista
> tipo tabla con todas las columnas". Nueva
> `migration-2026-09-09-client-sellercloud-profile.sql` — **YA CORRIDA en
> producción y `sellercloud-customers` YA REDESPLEGADA** (confirmado por el
> usuario el mismo día); queda solo el **deploy del frontend**. Segunda
> iteración del día a pedido del usuario ("cuando se expande la tabla no hay
> manera de scrollear a los lados"): en vista completa el cuadro de la tabla
> tiene alto acotado hasta el fondo de la ventana con scroll propio (las dos
> barras a la vista), encabezado y columna Nombre sticky, botones ◀ ▶ y las
> filas desplegadas sticky a la izquierda — ver punto 80. La migración era
> aditiva e iba ANTES del deploy del frontend (que inserta las columnas nuevas):
> `clients.business_name/sc_customer_group_id/sc_customer_group_name/
> sc_account_manager_id/sc_salesman/sc_comments`,
> `vendedores.sellercloud_group_id/sellercloud_group_name` (sembrados para las
> 12 vendedoras conocidas desde el export real) y RPC auditada
> `update_client_sc_profile`. Hallazgo clave: el AccountManagerId de los
> customers reales ES el `sellercloud_rep_id` de su vendedora (verificado en
> producción), así que el account manager se elige entre vendedoras. La Edge
> Function `sellercloud-customers` manda la ficha (create con BusinessName →
> UN PUT con teléfono + AccountManager1Id + Salesman + Comments → POST al
> grupo) y gana la acción `update` para clientes ya vinculados —
> **redesplegarla** (`supabase functions deploy sellercloud-customers`, sin
> secrets nuevos). Panel: ficha en el alta (prellenada por vendedora y lista),
> fila de edición desplegada, botón Vista completa (13 columnas entonces; 12
> desde el 2026-09-10, cuando el teléfono pasó debajo del nombre — punto 83), grupo
> editable en Vendedoras. Verificado: 16 bloques SQL en PG 18 + idempotencia +
> preflight negativo, `sc-customers-tests` 26 → 44, Playwright 122/122 (pescó
> un bug real de orden de declaración en el panel y un desborde previo del
> form de alta en móvil), build limpio.
> **PENDIENTE: migración + redeploy de la función + deploy del frontend.**)
>
> Antes: 2026-09-08 (**Excel de la lista
> de precios para el cliente**, punto 78, a pedido del usuario: los clientes
> que "se ponen tercos" con el Excel —los de más edad— pueden bajar SU lista
> completa desde el catálogo (bloque "📊 ¿Prefieres verlo en Excel?" arriba
> de los resultados) y la vendedora puede bajar la de cualquier lista desde la
> pestaña Precios para mandársela por WhatsApp. Puente temporal a propósito.
> Sin RPC ni migración: se arma en el navegador con lo que ya devolvió
> `get_catalog`, SheetJS bajo demanda. Cada descarga deja un `info` en
> `system_logs` (source nuevo `catalog`) para saber quién sigue pidiéndolo.
> Verificado: `tests/price-list-excel-tests.mjs` NUEVO (40, Node) + 43
> aserciones Playwright en el catálogo + 24 en Precios, build limpio.
> **PENDIENTE: deploy del frontend** (se suma al deploy ya pendiente).)
> Segunda tanda del mismo día (**correo del cliente visible y editable de
> nuevo en Clientes**, punto 79, a pedido del usuario: "poder ver qué correos
> están asignados a cada cliente registrado". Sin volver a la columna vacía
> que se quitó el 09-01: flechita **✉️ ▾** junto al teléfono (solo si hay
> correo) que despliega una fila debajo con el correo y **Copiar correo**;
> chips **✉️ N con correo / M sin correo** con filtro; input de correo al
> Editar la fila (vacío = quitar, viaja `null` a `update_client_info`). La
> primera versión lo ponía en línea bajo el teléfono y el usuario la
> rechazó: ensanchaba la columna y cortaba "Eliminar". Sin migración.
...
> export a Excel. Selector de rango 7/14/30 días, default 14 (desde 2026-09-11
> también "Histórico", punto 87).
...
- [x] **Edge Function `superadmin-users` desplegada** (2026-08-05, confirmado
  por el usuario; **desapareció de producción en algún momento antes del
  2026-08-19 — nadie sabe cómo — y se redesplegó el 2026-09-10, v2**, ver el
  aviso del encabezado y el punto 82): `supabase functions deploy superadmin-users`. Es la que
...
- [ ] **Recuperar un pedido perdido siempre crea una cotización** (2026-08-13,
  punto 59): `migration-2026-08-13-recover-as-quote.sql` **NUNCA CORRIÓ en
  producción** (descubierto el 2026-09-17: el doc la daba por corrida desde
  el 08-19 y el vivo seguía siendo el del 08-05; 89 recuperaciones entraron
  como pedidos reales). **La absorbe `migration-2026-09-17-recovery-notifications.sql`**
  (punto 91) y desde ese día tiene un guard: ya no se corre. Antes,
  `recover_order_failure` recreaba el `kind` original del
...
- [x] **Fallos sin cliente/ítems se pueden descartar** (2026-08-13, punto 60):
  `migration-2026-08-13-dismiss-order-failures.sql` **corrida en producción el
  2026-09-10** (verificado: columna y RPC existen; era la última pendiente —
  el frontend ya llevaba semanas desplegado sin ella, y el banner rojo se veía
  vacío en silencio). Había que correrla **junto con el deploy** — el frontend nuevo ya filtra por
  ...
  como `dismiss_order_failure`. Botón "Descartar" en el panel aparecía
  exactamente cuando "Recuperar" no aparecería (sin cliente, sin ítems, o
  ítems vacíos); **desde 2026-09-11 (punto 85) aparece en todos los fallos**
  — en los recuperables pasa por un modal de confirmación, en los otros sigue
  siendo directo. La RPC no cambió: nunca exigió que el fallo fuera
  irrecuperable.
...
- [x] **Sistema centralizado de logs de errores y eventos** (2026-08-20, punto
  ...
  aviso si la migración no corrió. **Desde 2026-09-09** (punto 81): chip
  "👤 Clientes creados desde el catálogo" (source `clients`, que emite
  `ClientsAdmin` en cada alta con la ficha por nombres y `created_by`),
  búsqueda de texto sobre lo cargado y "Exportar Excel" con TODAS las páginas
  del filtro (tope 5,000) y el context aplanado en columnas.
  `src/utils/systemLog.js` es la puerta del
...
| ~~`migration-2026-08-13-recover-as-quote.sql`~~ | ⛔ **NUNCA CORRIÓ — y ya NO correrla** (descubierto el 2026-09-17: el preflight de la 09-17 rechazó el recover vivo y `pg_get_functiondef` mostró la versión 08-05; la "verificación" del 08-19 fue falsa — misma lección que la 08-04). La absorbe `migration-2026-09-17-recovery-notifications.sql`; desde el 09-17 tiene un guard que aborta si el vivo ya es el de la 09-17 (correrla después la desharía) | "Recuperar" crea cotización y no pedido real (punto 59) — **vivo recién con la 09-17**. Hasta entonces las 89 recuperaciones (08-07 → 09-17) crearon pedidos reales con precio congelado: 34 atendidos, 50 cancelados, 5 nuevos |
| `migration-2026-08-13-dismiss-order-failures.sql` | ✅ corrida el **2026-09-10** por el usuario (verificado el mismo día: `order_failures.dismissed_at` y `dismiss_order_failure` existen). Entre el 08-19 y el 09-10 estuvo pendiente con el frontend ya desplegado | "Descartar" para fallos sin rescate posible (punto 60). Sin ella `loadFailures()` recibía `42703` y el banner rojo quedaba **vacío en silencio** (el `.then` descarta el error), no roto |
...
| `migration-2026-09-14-client-address.sql` | ⏳ **SIN CORRER** (2026-09-14, punto 89; probada ×2 + asserts + preflight negativo en PG 18 desechable) | dirección del cliente en `clients` (`address_line1/2`, `address_city`, `address_state`, `address_zip`, `address_country` ISO-2 con check) + `sc_address_id` / `sc_address_synced_at` / `sc_address_error`; RPC `update_client_address` (completa o vacía, auditada) y `mark_client_sc_address` (la anota la Edge Function); `sync_upsert_clients` acepta la dirección por fila (opcional, el payload actual de n8n sigue igual) + `sync_country_iso2`. Aditiva: puede correr antes del deploy sin romper el frontend viejo. **Va ANTES** del deploy del frontend (que inserta las columnas nuevas) y del redeploy de `sellercloud-customers` (v8) y `sellercloud-push-order` (v15) |
...
**Nota aparte, CONFIRMADA después**: la Edge Function `superadmin-users` devolvió
`404` donde `admin-create-vendedora-user` devuelve `403`, lo que sugiere que no
está desplegada — contradecía lo que el usuario confirmó el 2026-08-05, pero
`supabase functions list` lo confirmó el 2026-08-19 y seguía así el 2026-09-10:
**no estaba desplegada** (se borró en algún momento, no se sabe cómo). El
2026-09-10 se redesplegó (v2; sonda → `401`). Lección: la sonda HTTP a
`/functions/v1/<slug>` SÍ sirve — `404` es "no existe", `401`/`403` es "viva".
...
└── src/
    ...
    │   ├── PriceListExcel.jsx  ← bloque "📊 ¿Prefieres verlo en Excel?" (2026-09-08, puente temporal: baja la lista completa del cliente en .xlsx, SheetJS bajo demanda)
    │   ├── CartBar.jsx         ← barra inferior fija en móvil
    │   └── RecoveryBell.jsx    ← 🔔 avisos de recuperación en el header del panel, solo vendedora: Ver / Marcar visto / Marcar todos (2026-09-17; sin "Ver historial": el historial es del Registro de movimientos)
    ├── hooks/
    │   ├── useInfiniteRows.js  ← scroll infinito por lotes (tablas del panel)
    │   ├── useInventoryFreshness.js ← frescura de inventario (poll 60 s) + refresco vía Edge Function (2026-09-04)
    │   └── useRecoveryNotices.js ← avisos de recuperación pendientes de la vendedora (RLS), poll 60 s + visibilitychange, markSeen → mark_recoveries_seen, `unavailable` si falta la migración (2026-09-17)
    ├── pages/
    │   ├── Catalog.jsx         ← página principal (/?c=<token>)
    │   └── admin/
    │       ├── AdminLayout.jsx ← login Supabase Auth + nav de pestañas; monta useInventoryFreshness y useRecoveryNotices y los reparte por Outlet context (`{ role, isSuper, inventory, recoveries, userId }`); 🔔 solo para vendedora (2026-09-17)
    │       ├── ProductsAdmin.jsx
    │       ├── PricesUpload.jsx
    │       ├── ClientsAdmin.jsx
    │       ├── VendedoresAdmin.jsx
    ...
    └── utils/
        ...
        └── excel.js            ← parseSheet(), normalizeHeader(), pick(), downloadOrderExcel(), downloadAuditLogExcel(), downloadMetricsExcel(), priceListExcelRows()/buildPriceListWorkbook()/downloadPriceListExcel() + fileSlug() (XLSX lazy)
```
...
| Tabla | Descripción |
|---|---|
| `price_lists` | Listas de precio (7 sembradas, ver tabla abajo). **`min_order numeric(12,2)`** (2026-09-15, `migration-2026-09-15-price-list-min-order.sql`, punto 90): pedido mínimo en USD de la lista — null = sin mínimo (`quote`), default 800 para listas nuevas (`sa_create_price_list` no lo pide), check `>= 0`. Semilla solo la primera vez que corre la migración (800 / 2000 / null); un ajuste a mano sobrevive a re-correrla. Lo devuelve `get_catalog` (`client.min_order`) y lo hace cumplir `create_order` para `kind = 'order'` |
...
| `clients` | Clientes con token único, lista asignada y `vendedora_id` (FK a `vendedores`). Desde la v2 del sync (2026-07-10): `sellercloud_id` (integer unique nullable, el General.ID de SellerCloud — llave del sync automático; null en clientes cargados a mano/Excel) y `price_list_id` pasó a ser **nullable** (los clientes nuevos del sync entran sin lista; un cliente sin lista ve catálogo vacío y no puede pedir hasta que se la asignen a mano). **Ficha SellerCloud** (2026-09-09, `migration-2026-09-09-client-sellercloud-profile.sql`, todas nullable): `business_name` (→ BusinessName), `sc_customer_group_id` + `sc_customer_group_name` (grupo de clientes allá; el nombre es foto porque la API no lista grupos), `sc_account_manager_id` (→ AccountManager1Id; es el `sellercloud_rep_id` de una vendedora), `sc_salesman` (→ Salesman, texto), `sc_comments` (→ Comments: Mayorista/Minorista/Distribuidor/Zimaxx Box/Zimaxx Plus). Las escribe el alta (insert directo, RLS) y la RPC `update_client_sc_profile`; la Edge Function `sellercloud-customers` las lee de la fila para mandarlas. **Dirección** (2026-09-14, `migration-2026-09-14-client-address.sql`, todas nullable): `address_line1`, `address_line2`, `address_city`, `address_state`, `address_zip`, `address_country` (ISO-2 en mayúsculas, check `clients_address_country_iso2`) — UNA dirección, envío = facturación, obligatoria solo para crear el customer allá — más `sc_address_id` (UserAddressDto.ID una vez verificado releyendo), `sc_address_synced_at` (null con dirección y vínculo = pendiente de mandar) y `sc_address_error` (último fallo del PUT/verificación; null = ok). Las escriben el alta (insert), la RPC `update_client_address`, la RPC `mark_client_sc_address` (desde la Edge Function) y `sync_upsert_clients` (n8n, como sincronizada) |
| `vendedores` | Nombre + teléfono de cada vendedora (2026-07-06; antes texto libre en `clients`). Desde el rol vendedora (2026-07-06): `user_id` (FK a `auth.users`, nullable, único) + `login_email` (solo display) para vincular su login. `sellercloud_rep_id` (2026-08-18, integer nullable, `migration-2026-08-18-sellercloud-salesrep.sql`): ID de empleado en SellerCloud (Settings → Employees), viaja como `OrderDetails.SalesRepresentative` al mandar un pedido con "Enviar a SellerCloud"; editable inline en la pestaña Vendedoras; null = la orden entra sin Sales Rep, con aviso. **Desde 2026-09-09 es también el account manager** que se propone para sus clientes (en los customers reales AccountManagerId = rep_id de la vendedora, verificado en producción). `sellercloud_group_id` + `sellercloud_group_name` (2026-09-09, `migration-2026-09-09-client-sellercloud-profile.sql`): el grupo de clientes de esa vendedora en SellerCloud (Customers → Groups), propuesto como grupo al crear un cliente suyo; editables juntos en la pestaña Vendedoras; sembrados por nombre para las 12 vendedoras conocidas (solo donde estaba null) |
...
| code | label | `min_order` (2026-09-15) |
|---|---|---|
| `us_min` | US Minimum Order ($800+) | 800 |
| `us_wholesale` | US Wholesale ($2,000+) | 2000 |
| `ve_min` | VE Minimum Order | 800 |
| `ve_wholesale` | VE Wholesale | 2000 |
| `special` | Special Order ($15,000+, **cualquier región**, precio fijo real) | 2000 (regla literal: no es el catálogo de $800) |
| `quote` | Cotización (sin precio) — catálogo completo sin precio, ver más abajo | null (sin mínimo) |
| `luzmar` | Luzmar - Precio Especial (2026-07-09, lista exclusiva de Luzmar Quintero, sin lógica especial — precio real como cualquier otra lista; en producción el label es "Lista VIP") | 2000 (ídem; bajar a 800 es un `update` de una línea) |
...
- Desde 2026-09-15 (`migration-2026-09-15-price-list-min-order.sql`) el
  objeto `client` trae **`min_order`**: el pedido mínimo de la lista del
  cliente (`price_lists.min_order`; `null` = sin mínimo, lista `quote`). El
  carrito lo usa para el aviso "El pedido mínimo de tu lista es $X · Te
  faltan $Y" y para bloquear el envío por WhatsApp; si la clave no viene
  (base sin migrar) el frontend asume el 800 de siempre (`src/utils/minOrder.js`).
...
- **2026-09-15 — pedido mínimo por lista** (`migration-2026-09-15-price-list-min-order.sql`,
  punto 90). Para `kind = 'order'`, si la lista del cliente tiene
  `min_order` y el total **recalculado** queda por debajo, no se guarda:
  fila en `order_failures` con motivo `total $1,900.00 por debajo del pedido
  mínimo de $2,000.00 de la lista US Wholesale` (+ payload, kind y
  line_count) y `return null` → el carrito muestra el aviso de "rechazado".
  Va después del rechazo por producto sin precio (ese tiene prioridad) y
  antes del insert. Borde inclusivo (total = mínimo entra). Las cotizaciones
  (PDF, lista `quote`) no tienen mínimo. `p_total` del navegador sigue
  ignorado. `create_manual_order`, `convert_quote_to_order` y
  `update_order_items` NO lo chequean: ahí decide una persona.
...
- Marca la fila con `recovered_order_id` y audita en `admin_audit_log`
  ...
  **Desde 2026-09-17** (`migration-2026-09-17-recovery-notifications.sql`,
  punto 91 — que además ABSORBE la 08-13: en producción el vivo hasta ese día
  era el del 08-05 y recuperaba con el kind original) graba en la fila el
  registro y el aviso: `recovered_at =
  now()`, `recovered_by = auth.uid()`, `recovered_by_email`, y
  `recovery_seen_at` = `now()` si quien recupera es la propia dueña del
  cliente o el cliente no tiene vendedora (`v_client.vendedora_id is not
  distinct from current_vendedora_id()` — no hay a quién avisar), null si lo
  recuperó otra persona (aviso pendiente para la campanita). El update es
  compare-and-set (`and recovered_order_id is null` + `if not found → 'este
  pedido ya fue recuperado'`): dos clicks simultáneos ya no crean dos
  cotizaciones. El `detail` de la auditoría suma `vendedora_id` y
  `notify_vendedora` (true = quedó aviso pendiente) para que el Registro de
  movimientos lo diga. Devuelve `{ok, order_id, total, lines, recovered_at}`.
- La llama el aviso rojo de `OrdersAdmin.jsx` (botón "Recuperar").

### `mark_recoveries_seen(p_failure_ids uuid[]) → integer` (2026-09-17)
- Acceso: solo `authenticated`; mismo gate que recover/dismiss (`is_admin()`
  o `is_vendedora()`, cualquier otro → `'no autorizado'`; `anon` sin EXECUTE →
  42501).
- "Marcar visto" de la campanita (uno o todos). `update order_failures
  set recovery_seen_at = now()` solo sobre filas `id = any(p_failure_ids)`
  **recuperadas, pendientes y de los clientes de la vendedora que llama**
  (`client_id in (select id from clients where vendedora_id =
  current_vendedora_id())`). Devuelve las filas afectadas: un admin puro
  recibe 0 (no tiene avisos propios), una vendedora no puede marcar los de
  otra, marcar dos veces devuelve 0 la segunda; `null`/vacío → 0; más de 500
  ids → excepción. **Audita cada visto** en `admin_audit_log` como
  `recovery_seen` (una fila por fallo: `client_id`/`client_name`, `order_id` =
  el pedido recuperado, `detail = {failure_id, kind, reason, recovered_at,
  recovered_by_email}`), en la MISMA sentencia que el update (`with upd as
  (update … returning …) insert …`): se audita exactamente lo que se marcó y
  nada si no se marcó nada (a pedido del usuario, segunda vuelta del 09-17:
  "eso debe estar en registro de movimientos, identificado como tal"). Sin RLS
  nueva.
- La llaman `useRecoveryNotices.js` (campanita: uno o todos) y el cuadro
  verde de `OrdersAdmin.jsx`.
...
- Devuelve **todo lo que dibuja la pestaña en un solo `jsonb`**, porque son 7
  ...
  - `period` → `{ days, from, to, historic, since, bucket }`. `p_days > 0` se
    clampea a `[1, 365]` (`coalesce(p_days, 14)`), así un valor a mano no
    genera un `generate_series` gigante. **`p_days = 0` = Histórico**
    (2026-09-11, `migration-2026-09-11-sa-metrics-historic.sql`, punto 87):
    `from` = 00:00 del primer pedido real (`since`, excluidas las cuentas de
    prueba; null y ventana de hoy si no hay ninguno), `to` = ahora, sin clamp.
    `bucket` = `'day'` hasta 120 días y `'week'` de ahí en adelante: la
    `serie_diaria` sale entonces por semana (`dia` = lunes) y el total sigue
    siendo exactamente su suma. Con la RPC anterior `p_days = 0` se clampea a
    1 y `period` no trae `historic`: el frontend lo detecta y avisa que falta
    la migración.
...
### `update_client_sc_profile(p_client_id uuid, p_business_name text, p_customer_group_id integer, p_customer_group_name text, p_account_manager_id integer, p_salesman text, p_comments text) → jsonb` (2026-09-09)
- Edita la **ficha SellerCloud** del cliente (`migration-2026-09-09-client-sellercloud-profile.sql`): empresa, grupo (ID + nombre), account manager, salesman y comentarios — los seis parámetros son el valor NUEVO completo (el form siempre manda todo); vacío/null = quitar. SECURITY DEFINER, `authenticated`.
- Permisos: admin cualquiera; **vendedora solo sus propios clientes y solo con SU grupo y ella misma como account manager** (`vendedores.sellercloud_group_id` / `sellercloud_rep_id` de `current_vendedora_id()`; quitar con null sí puede). Asignar el account manager de otra vendedora es tocar comisiones ajenas — nivel `reassign_client`, solo admin. Mensajes: 'solo podés asignar tu propio grupo de SellerCloud' / 'solo podés asignarte a vos misma como account manager'.
- Reglas: IDs enteros positivos o null; sin grupo el nombre queda null (nada de nombres huérfanos); con grupo y sin nombre, lo busca en `vendedores` por `sellercloud_group_id` (si ninguna lo tiene, queda null — no inventa). Trim en los textos.
- **Solo lo que cambió** va a la auditoría: acción `'update_client_sc_profile'` ("Ficha SellerCloud" en el Registro) con `from_x/to_x` únicamente de las claves cambiadas + `changes` (array: business_name / customer_group / account_manager / salesman / comments). Sin cambios → `{ok, changed: false}` sin fila.
- Devuelve `{ok, changed, changes}`; `ClientsAdmin.jsx` usa `changed` para decidir si llama a la acción `update` de la Edge Function (solo si el cliente ya tiene `sellercloud_id`).
- NO empuja nada a SellerCloud: eso lo hace la Edge Function con el JWT del caller después, mejor-esfuerzo. Probada en PG 18 desechable (16 bloques: columnas y seed, grants, admin completo/no-op/solo-lo-cambiado/vaciar/grupo desconocido/cliente sin vendedora, validaciones, vendedora propia/quitar/grupo ajeno/AM ajeno/cliente ajeno/sin asignar, sin rol, insert directo con ficha) + re-aplicación idempotente (no pisa un grupo corregido a mano) + preflight negativo.

### `update_client_address(p_client_id uuid, p_line1 text, p_line2 text, p_city text, p_state text, p_zip text, p_country text) → jsonb` (2026-09-14)
- Edita la **dirección** del cliente (`migration-2026-09-14-client-address.sql`): los seis parámetros son el valor NUEVO completo (el form siempre manda todo). SECURITY DEFINER, `authenticated`; admin cualquiera, vendedora solo sus propios clientes.
- Regla central: la dirección viene **COMPLETA** (calle, ciudad, estado, código postal, país — el estado es obligatorio para todos los países por decisión del usuario) **o toda vacía**, nunca a medias: una dirección a medias es justo lo que SellerCloud rechaza y lo que hace rebotar las órdenes. Falta algo → `dirección incompleta: falta …` con la lista. País como ISO-2 (se pasa a mayúsculas; otra cosa → error), zip de 3 a 12 caracteres, estado de US en mayúsculas si son dos letras.
- Vaciarla solo si todavía no está en SellerCloud (`sc_address_id` null): "este cliente ya tiene dirección en SellerCloud: se puede corregir, no borrar".
- Cualquier cambio deja `sc_address_synced_at` en null ("pendiente de mandar"): lo que hay allá quedó viejo hasta que la Edge Function la vuelva a cargar y verificar. `sc_address_id` se conserva (el reintento actualiza esa misma dirección en vez de duplicarla).
- **Solo lo que cambió** va a la auditoría: `'update_client_address'` ("Dirección del cliente" en el Registro) con `from_x/to_x` de las claves cambiadas + `changes` (line1 / line2 / city / state / zip / country) + `sellercloud_id`. Sin cambios → `{ok, changed: false}` sin fila.
- Devuelve `{ok, changed, changes, complete}`. NO empuja nada a SellerCloud: eso lo hace la acción `update` de la Edge Function (la que ya empuja la ficha) o el botón "Enviar dirección" de la fila. Probada en PG 18 desechable (punto 89).

### `mark_client_sc_address(p_client_id uuid, p_sc_address_id integer, p_error text) → jsonb` (2026-09-14)
- Anota el resultado de `PUT /Customers/{id}/Addresses` + relectura. La llama la Edge Function `sellercloud-customers` con el JWT de quien apretó el botón (misma regla de permisos: admin cualquiera, vendedora sus clientes; `authenticated`).
- `p_error` null = la dirección está allá y se verificó: guarda `sc_address_id` (entero positivo; null conserva el anterior), `sc_address_synced_at = now()`, limpia `sc_address_error` y audita `'sync_client_address'` ("Dirección enviada a SellerCloud") con la dirección completa, el `sellercloud_id` y el ID de la dirección. Devuelve `{ok, synced: true, sc_address_id}`.
- `p_error` con texto (recortado a 2000) = no entró: `sc_address_error` = el motivo, `synced_at` null, sin auditar (el detalle largo ya va a `system_logs`). Devuelve `{ok, synced: false}`. El panel lo muestra en rojo con "Reintentar dirección".
- `sync_upsert_clients(p_rows jsonb)` (n8n) también escribe estas columnas desde el mismo día: si la fila trae `address/city/zip/country` utilizables (país reconocido por `sync_country_iso2`), guarda la dirección COMO SINCRONIZADA y pisa lo local — para un customer que ya existe allá SellerCloud es la fuente de verdad; si no, descarta la dirección y no la fila (`addresses` / `addresses_skipped` en el resumen).
...
| `VITE_MIN_ORDER` | `.env` local + Netlify (opcional) | **Solo fallback** del pedido mínimo (default 800). Desde 2026-09-15 el mínimo real es por lista (`price_lists.min_order` → `client.min_order` de `get_catalog`); esto aplica únicamente si la base no tiene la migración del 09-15 |
...
- **Descargar Excel de la lista** (2026-09-08): botón a la derecha de los
  contadores de la matriz. Genera la lista elegida en el selector tal como la
  ve un cliente de esa lista (activo y precio > 0 en ESA lista, orden marca →
  nombre — el criterio de `get_catalog`), con el mismo generador que el bloque
  "📊 ¿Prefieres verlo en Excel?" del catálogo (`priceListExcelRows` +
  `buildPriceListWorkbook` en `utils/excel.js`) más la columna SKU. Para
  mandárselo por WhatsApp a los clientes que prefieren el Excel al link. Lo
  ven admin y vendedora; sin lista elegida pide elegir una; lista sin
  productos con precio → aviso, sin archivo. Sin request: usa lo que la matriz
  ya tiene cargado. Ver punto 78.
...
- **Ficha SellerCloud** (2026-09-09, fieldset "📦 Ficha SellerCloud" en el
  mismo form): Empresa, Grupo (select con los grupos cargados en
  `vendedores.sellercloud_group_id`), Account manager (select con las
  vendedoras que tienen `sellercloud_rep_id`), Salesman (input con datalist
  de nombres de vendedoras) y Comentarios (input con datalist Mayorista /
  Minorista / Distribuidor / Zimaxx Box / Zimaxx Plus). **Prellenado**: al
  elegir vendedora (o si la lista impone una dueña) grupo/account
  manager/salesman toman los de esa vendedora; la vendedora logueada arranca
  con los suyos (solo ve su fila, así que sus selects tienen una opción). Al
  elegir lista el comentario se propone por nivel (`commentsForListCode`:
  `*_min` → Minorista, `*_wholesale` → Mayorista, `special` → Distribuidor)
  salvo que el campo tenga un texto propio (no sugerido). Va en el mismo
  insert (`business_name`, `sc_*`, con `sc_customer_group_name` resuelto
  desde las opciones) aunque el toggle de SellerCloud esté apagado — queda
  lista para cuando se cree/vincule. La Edge Function la lee de la fila.

### Edición de nombre/teléfono del cliente (2026-08-25; vínculo SellerCloud 2026-08-31; columna Email quitada 2026-09-01; correo visible y editable de nuevo 2026-09-08; ficha SellerCloud y vista completa 2026-09-09)
- **Vista completa** (2026-09-09, a pedido del usuario): botón **⤢ Vista
  completa / ⤡ Vista compacta** junto a "+ Nuevo cliente" (`aria-pressed`,
  preferencia en `localStorage` `zimaxx_clients_view`). La compacta es la de
  siempre, **5 columnas desde 2026-09-10** (Nombre, SellerCloud, Lista,
  Vendedora, acciones — eran 6: la columna Tel se quitó a pedido del usuario y
  **el teléfono va debajo del nombre**, en mono chico, con la flechita ✉️
  pegada; al editar, los inputs de nombre, teléfono y correo van apilados en
  esa misma celda). La completa suma Empresa, Email (columna propia — la
  flechita ✉️ y su fila desplegable son solo de la compacta), Grupo (nombre,
  `title` con el #id), Account manager (nombre resuelto por `sellercloud_rep_id`
  + #id), Salesman, Comentarios y Alta (fecha `created_at` en el idioma del
  panel): 12 columnas (eran 13), `colCount` alimenta los `colSpan` de las filas
  desplegadas. La tabla scrollea dentro del `overflow-x-auto`; la página no
  (verificado a 1400 y a 390 px). El buscador matchea además empresa, grupo,
  salesman y comentarios. **Desplazamiento lateral** (misma fecha, segunda
  iteración): el cuadro tiene alto acotado hasta el fondo de la ventana
  (medido en cada render) con scroll vertical propio, así la barra horizontal
  queda siempre a la vista; encabezado y columna Nombre sticky; fila de pista
  con botones ◀ ▶ (320 px); las filas desplegadas quedan sticky a la
  izquierda; el pie con el conteo está fuera del cuadro. Nada de esto aplica
  en la compacta.
- **Ficha SellerCloud en edición** (2026-09-09): al tocar Editar, además de
  los inputs en línea de siempre, se despliega una fila bajo el cliente
  (`sc-profile-edit-row`, en las dos vistas) con los cinco campos de la
  ficha prellenados con lo guardado (Enter guarda, Escape cancela, mismos
  botones Guardar/Cancelar de la fila) y el aviso "Cambiar el grupo lo agrega
  al nuevo en SellerCloud; del grupo anterior hay que quitarlo allá". Al
  guardar: primero `update_client_info` (y `set_client_sellercloud_id` si
  admin cambió el ID), después — solo si la ficha cambió — la RPC
  `update_client_sc_profile`, y si el cliente tiene `sellercloud_id`, la
  acción `update` de la Edge Function `sellercloud-customers` la empuja
  (mejor-esfuerzo). Banner `role="status"` verde "✓ Ficha actualizada en
  SellerCloud (#id)" o ámbar "⚠️ …" con el `warning` de la función / "Guardado
  acá, pero no se pudo actualizar en SellerCloud: …" / "Guardado acá. El
  cliente no está vinculado…", con ✕ para cerrarlo. Si la RPC de la ficha
  falla, nombre/teléfono/correo ya quedaron guardados (reflejados) y la
  edición sigue abierta con el error rojo. Sin cambios en la ficha no se llama
  a nada. El form "Crear en SellerCloud" del panel de la fila muestra la misma
  ficha (lo guardado o, si no hay nada, lo de su vendedora + comentario por su
  lista) y la guarda por la RPC ANTES de invocar el create.
- **Correo** (2026-09-08, a pedido del usuario): junto al teléfono, solo si
  el cliente tiene correo, una flechita **✉️ ▾** (aria-label "Ver correo" /
  "Ocultar correo") despliega una fila debajo con el correo completo y un
  botón **Copiar correo** ("✓ Copiado" 1.5 s, igual que Copiar link); varias
  filas abiertas a la vez (`emailOpen` es un Set). Chips **✉️ N con correo /
  M sin correo** junto a los filtros — contadores sobre lo que ya filtran
  buscador/lista/vendedora, y al tocarlos recortan la tabla. Al **Editar**
  aparece el input de correo debajo del de teléfono: chequeo laxo
  (`EMAIL_RE`, el mismo del alta), minúsculas, vacío = quitar (viaja `null` a
  `update_client_info`; antes se reenviaba el guardado tal cual porque no
  había input). Admin y vendedora. NO va en línea ni en columna aparte a
  propósito: la primera versión del día lo ponía bajo el teléfono y el
  usuario la rechazó porque ensanchaba la columna y "Eliminar" quedaba
  cortado; la celda de acciones lleva `whitespace-nowrap` desde entonces.
  Ver punto 79.
...
- Columna **SellerCloud** (2026-08-18): ID de empleado, editable en un click
  (ver `sellercloud_rep_id`). Desde 2026-09-09 ese mismo ID es el **account
  manager** que se propone para sus clientes.
- Columna **Grupo SellerCloud** (2026-09-09): `#id nombre` (o "sin grupo"),
  editable en un click con dos inputs (ID numérico + nombre) y ✓ — Enter
  guarda, Escape cancela; ID vacío = sin grupo (borra también el nombre); ID
  sin nombre no guarda y avisa ("Poné el nombre del grupo junto con el ID."),
  porque la API de SellerCloud no permite resolver el nombre después y es lo
  que muestran la ficha y la vista completa de Clientes. Update directo a
  `vendedores` (admin_all), sin RPC, igual que el rep_id. El ID sale de
  SellerCloud → Customers → Groups.
...
- **Aviso de pedidos que no se registraron** (2026-08-05): bloque rojo arriba de
  ...
  qué reconstruir el pedido. **Desde 2026-09-10 es un cuadro expandible**
  (punto 84): plegado por defecto con el conteo total y por tipo ("N Pedidos ·
  M Cotizaciones") y botón "Ver detalle ▾"; dentro, cada fallo lleva el badge
  **Pedido/Cotización** (`order_failures.kind`) y un botón **"Ver contenido ▾"**
  que despliega Producto · Cantidad (y precios solo si el payload los traía),
  resolviendo sku/nombre desde `products` para los payloads slim del outbox y
  marcando en rojo las líneas que nombra el motivo "sin precio en la lista".
  **Desde 2026-09-11** (punto 85): cada fallo lleva su **fecha en línea
  propia**, en el idioma del panel y con día relativo ("📅 mar, 8 sept. 2026,
  03:00 p. m. · hace 3 días" / "ayer" / "hoy"), el motivo pasa a la línea de
  abajo, y el resumen plegado dice el **rango de fechas** ("📅 del 12 ago. 2026
  al 11 sept. 2026"). Y **"Descartar" aparece en todos los fallos**, también en
  los recuperables (junto a "Recuperar"): en esos abre un modal que dice de
  quién y de cuándo es el intento y que desde el panel no se deshace; sin
  cliente/ítems sigue siendo directo. `created_at` es cuando `create_order`
  rechazó el intento; para los del outbox expirado es cuando el teléfono lo
  reportó (≥24 h después), no cuando el cliente tocó Enviar. Tope de filas del
  cuadro 50 → 200 (`FAILURES_CAP`); si se llega, el resumen lo dice.
- **Chip "♻️ Recuperado" por fila + campanita 🔔** (2026-09-17, punto 91, a
  pedido del usuario): la fila de una cotización que salió de una
  recuperación lleva el chip, con tooltip "recuperado el {fecha} · por {correo
  | vos}". Lo alimenta `loadRecovered()` — filas de `order_failures` con
  `recovered_order_id` dentro de la ventana de la bandeja (sobre
  `recovered_at`), consulta separada de `loadFailures` (42703 → sin chips, el
  rojo sigue). La campanita vive en `AdminLayout` (`RecoveryBell.jsx`, estado
  en `useRecoveryNotices.js`: pendientes de SUS clientes vía RLS, poll 60 s +
  `visibilitychange`, tope 50 → "50+"), solo para `role === 'vendedora'`;
  "Ver" navega a `/admin/orders?recovered=<order_id>` y la bandeja lo atiende
  al terminar de cargar (limpia filtros, despliega `tr#order-<id>`, hace
  scroll; si está fuera de la ventana amplía a "todo el historial" y
  reintenta; si tampoco, aviso ámbar) y limpia la URL. El visto es explícito
  y se marca SOLO desde la campanita (uno o todos); una autorrecuperación
  nace vista (RPC). **Hubo un cuadro verde "♻️ Pedidos recuperados"** con el
  historial (conteo, rango, "Nuevo", "Marcar visto", "Ver cotización") **y el
  usuario pidió quitarlo el mismo día**: "quiero que el registro quede nada
  más en el registro de movimientos… el panel verde que se ve en pedidos
  quitalo de ahí, el rojo donde se recuperan los pedidos, ese sí lo puedes
  dejar". El historial está en 🛡️ Registro de movimientos (ver más abajo).
...
**Recuperaciones identificadas como tal (2026-09-17, punto 91, a pedido del
usuario: "cambia el registro de los pedidos, eso debe estar en registro de
movimientos, identificado como tal").** La etiqueta de `recover_order_failure`
no sale de la acción sino del `detail` (`actionLabel(a)` recibe la fila, no el
nombre): `kind = 'quote'` + `original_kind = 'order'` → **"Pedido recuperado
como cotización"**; `original_kind = 'quote'` → **"Cotización recuperada"**;
`kind = 'order'` (las 89 filas anteriores a la migración, versión 08-05 de la
RPC, sin `original_kind`) → **"Pedido recuperado como pedido real"**. Detalle:
"Intento original: Pedido/Cotización · motivo del rechazo · N ítems · total ·
aviso a la vendedora: pendiente / sin aviso (la recuperó la vendedora dueña o
el cliente no tiene vendedora)" — lo último solo si la fila trae
`notify_vendedora` (clave nueva; las viejas no dicen nada). Acción nueva
**`recovery_seen`** ("Aviso de recuperación visto", verde esmeralda): la
escribe `mark_recoveries_seen`, una fila por fallo marcado, con
`client_name`, `order_id` = el pedido recuperado y `detail = {failure_id, kind,
reason, recovered_at, recovered_by_email}`; el detalle dice "Cotización ·
recuperado el {fecha} por {correo}". Y `dismiss_order_failure` (2026-08-13)
por fin tiene etiqueta y estilo propios, **"Intento descartado"** (motivo +
hint del token): hasta hoy no estaba en `ACTION_LABELS`/`ACTION_STYLES` y sus
filas se pintaban como reasignación. Las tres entran en `ACTION_FILTERS` y en
el Excel (que usa el mismo `actionLabel(a)`/`detailText(a)`). El filtro de
recuperaciones se llama "Recuperación de pedido perdido" (`actionRecoverOrder`).
...
79. **Correo del cliente visible y editable de nuevo en Clientes** (2026-09-08, segunda tanda del día, a pedido del usuario: "poder ver qué correos están asignados para cada cliente registrado"). Contexto: la columna Email se había quitado el 2026-09-01 porque 0 de 2755 clientes tenían correo y solo mostraba "—"; desde entonces lo llenan el alta (campo opcional), el Excel de clientes, el panel SellerCloud (form de crear con correo) y el sync si n8n manda `email`. **Decisión (segunda iteración, misma sesión)**: NO volver a la columna aparte (seguiría casi vacía) y **tampoco en línea** — la primera versión lo mostraba bajo el teléfono (encabezado "Tel · Email", `truncate` + `title`) y el usuario la rechazó en la misma sesión: "el botón de eliminar está quedando corrido a la derecha y cortado"; pidió "una pequeña flechita que expanda un contenedor donde se va a poder ver el correo, incluso un botón para copiar". Quedó así: junto al teléfono, **solo si el cliente tiene correo**, un botón **✉️ ▾/▴** (`aria-expanded`, `aria-label`/`title` "Ver correo"/"Ocultar correo") que abre una **fila propia debajo del cliente** (`colSpan={6}`, mismo gesto que el panel SellerCloud) con "✉️ Email: <correo>" en mono `select-all` y un botón **Copiar correo** (`navigator.clipboard.writeText`, "✓ Copiado" 1.5 s — calcado de `copyLink`); `emailOpen` es un `Set` para poder tener varias abiertas; la celda de acciones ganó `whitespace-nowrap` para que Editar/Eliminar nunca se partan ni recorten (si no entra, scrollea el `overflow-x-auto`). Junto a los filtros van dos chips con contador, **✉️ N con correo / M sin correo**, calculados sobre `base` (lo que ya filtran buscador + lista + vendedora, ANTES del filtro de correo — así dicen cuántos de los que se están mirando tienen correo) y que al tocarlos recortan la tabla (`emailFilter` '' | 'has' | 'missing', también en las deps de `useInfiniteRows`). El buscador ya matcheaba por correo desde el 08-31; el placeholder ahora lo dice. **Edición**: al Editar la fila aparece el input `type="email"` debajo del de teléfono (placeholder "Email (opcional)"), `editForm.email` arranca del guardado; al guardar se valida con el mismo `EMAIL_RE` laxo del alta (inválido → `invalidEmail` en el banner, sin llamar a la RPC, la edición sigue abierta), se manda en minúsculas y **vacío viaja como `null`** a `update_client_info` — quitar el correo pasa a ser una acción explícita del input (antes, sin input, se reenviaba el guardado tal cual para no borrarlo). El estado local se refleja sin recargar (también en la rama en que falla `set_client_sellercloud_id` tras guardar nombre/teléfono/correo). Lo ven y editan admin y vendedora (el permiso real lo valida la RPC: admin cualquiera, vendedora sus clientes); el input de SellerCloud ID sigue siendo solo admin. i18n `withEmail`/`withoutEmail`/`showEmail`/`hideEmail`/`copyEmail` + `searchClients` actualizado (es/en). Sin migración ni RPC nueva. **Verificado**: 41 aserciones Playwright contra el build real con Supabase interceptado (encabezado "Tel" pelado y NADA en línea, flechita solo en quien tiene correo, `aria-expanded`, la tabla sin scroll horizontal a 1400 y a 1280 px con "Eliminar" dentro del contenedor —también con un correo de 70 caracteres desplegado—, desplegar → fila con el correo y la etiqueta, Copiar → "✓ Copiado" + portapapeles real (permisos clipboard en el contexto) + vuelve solo, dos abiertas a la vez y cerrar una deja la otra, chips con conteo y filtro, buscador por un pedazo del correo, agregar correo → RPC con trim+minúsculas + aparece la flechita + contador, inválido → aviso sin RPC, vaciar → `p_email: null` + desaparece la flechita, sin errores JS; rol vendedora despliega/copia/edita sin el input de SellerCloud ID); build de Vite limpio. **Pendiente: deploy del frontend** (mismo deploy que el punto 78).

80. **Ficha SellerCloud del cliente (empresa, grupo, account manager, salesman, comentarios) + vista completa de Clientes** (2026-09-09, a pedido del usuario: "el cliente se crea desde el catálogo todo bien, pero tenemos que actualizar los campos con los que se registran los clientes: business name, customer group, account manager, salesmen y comments … rediseñar el panel de clientes para que se puedan ver todos esos campos … un botón de expandir que cambie a una vista tipo tabla donde se puedan ver todas las columnas"). **Contrato de la API** (Swagger real del servidor, bajado ese día a `swagger.json`): `POST /Customers` acepta `BusinessName`; `PUT /Customers/{id}` (UpdateCustomerRequest) acepta `BusinessName`, `AccountManager1Id` (int), `Salesman` (texto), `Comments` (texto) y `Phone1`; `POST /Customers/CustomersGroups/{id}/Customers {CustomersIDs:[…]}` agrega a un grupo; **no existe** endpoint para listar grupos, quitar de un grupo ni listar empleados. **Lo que dijeron los datos reales** (export `Clientes detalle.txt`, 887 customers): cada vendedora tiene UN grupo "Clientes <Vendedora>" (IDs 3, 4, 5, 6, 7, 9, 13, 17, 18, 19, 20, 21), `Internal.SalesMan` es el nombre de la vendedora, `Internal.Comment` es el tipo de cliente (Mayorista 439 / Minorista 279 / Inactive 63 / Distribuidor 35 / Zimaxx Box 12 / Zimaxx Plus 4 + typos) y — hallazgo clave — `Internal.AccountManagerId` **es exactamente el `vendedores.sellercloud_rep_id`** de su vendedora (verificado uno a uno contra producción con `supabase db query --linked`: 75448/1040357/1040356/1149304/1694132/75431/1694131/1694133/75427/79963/75429/75439). Decisión: el account manager NO es un ID nuevo que cargar, se elige entre las vendedoras con rep_id; el grupo vive por vendedora (`vendedores.sellercloud_group_id/name`, editable en la pestaña Vendedoras porque la API no lo lista) y la ficha por cliente es la FOTO de lo que se mandó (o se va a mandar). **Migración `migration-2026-09-09-client-sellercloud-profile.sql`** (aditiva, preflight, idempotente; **SIN CORRER en producción** — va ANTES del deploy del frontend, que inserta las columnas nuevas): `clients.business_name/sc_customer_group_id/sc_customer_group_name/sc_account_manager_id/sc_salesman/sc_comments`, `vendedores.sellercloud_group_id/sellercloud_group_name` con **seed** por nombre (lower) para las 12 vendedoras conocidas solo donde esté null (re-correr no pisa un grupo corregido a mano), y RPC `update_client_sc_profile(7 params)` — admin cualquiera, vendedora sus clientes y **solo con SU grupo / ella como account manager** (comisiones ajenas = nivel reassign), sin grupo → nombre null, grupo sin nombre → lo resuelve desde vendedores o queda null, auditada `'update_client_sc_profile'` con `from_x/to_x` SOLO de lo que cambió + `changes[]`, no-op sin fila. **`sellercloud.ts`**: `customerUpdateBody` (solo claves con valor — el PUT toma lo ausente como "no tocar", mandar null borraría lo cargado allá), `updateCustomer` (un PUT con teléfono + ficha, devuelve las claves que viajaron, no llama si no hay nada), `setCustomerPhone` pasa a ser un caso de `updateCustomer`, `addCustomerToGroup` (POST al grupo, valida ID > 0 antes de llamar), `createCustomer` ya mandaba `business`. **Edge Function `sellercloud-customers`**: `loadClient` lee la fila con las columnas nuevas y **cae al select viejo si dan 42703** (función desplegada antes de la migración = ficha vacía, nunca alta rota); `create` → createCustomer con BusinessName → `pushProfile` (UN PUT teléfono+ficha → POST grupo, mejor-esfuerzo: warnings concatenados, se vincula igual) → RPC link; **acción nueva `update`** para un cliente ya vinculado (409 sin vínculo; error 502 solo si nada entró y hubo fallo; `update_ok` info/warning, `update_failed` error en system_logs). **Redesplegar** `supabase functions deploy sellercloud-customers` (sin secrets nuevos). **Frontend (`ClientsAdmin.jsx`)**: `PROFILE_KEYS`/`COMMENT_SUGGESTIONS`/`commentsForListCode`/`profilePayload`/`profileForm`; fieldset "📦 Ficha SellerCloud" en el alta (Empresa, Grupo select = grupos cargados en vendedoras, Account manager select = vendedoras con rep_id, Salesman input + datalist de vendedoras, Comentarios input + datalist) **prellenado** al elegir vendedora (`profileFromVendedora`, también cuando la lista impone dueña) y lista (`autoComments`: solo si el campo está vacío o es una sugerencia — un texto propio no se pisa); la vendedora logueada arranca con los suyos; va en el mismo insert con `sc_customer_group_name` resuelto; **fila de edición desplegada** (`sc-profile-edit-row`) bajo el cliente al tocar Editar, Enter/Escape como los inputs en línea; `saveProfile` = RPC solo si cambió (`profileChanged`) → si hay `sellercloud_id`, Edge Function `update` → banner `role="status"` verde/ámbar con ✕ (`actionNotice`); el form "Crear en SellerCloud" del panel muestra la ficha con `profileWithDefaults` (guardado → vendedora → comentario por lista) y la guarda por la RPC ANTES del create; **botón ⤢ Vista completa / ⤡ Vista compacta** (`fullView`, `localStorage` `zimaxx_clients_view`, `aria-pressed`): compacta = 6 columnas de siempre, completa = 13 (Nombre, Empresa, Tel, Email, SellerCloud, Grupo, Account manager nombre + #id, Salesman, Comentarios, Lista, Vendedora, Alta), `colCount` para los colSpan, flechita ✉️ solo en compacta; buscador matchea empresa/grupo/salesman/comentarios. **`VendedoresAdmin.jsx`**: columna Grupo SellerCloud (ID + nombre juntos, ✓/Enter/Escape, ID sin nombre no guarda y avisa). **`AuditLogAdmin.jsx`**: acción `update_client_sc_profile` ("Ficha SellerCloud") con detalle "Etiqueta: antes → después" solo de lo cambiado. i18n `businessName`/`scGroup*`/`scAccountManager*`/`scSalesman`/`scComments`/`scProfile*`/`view*`/`createdAtCol`/`actionUpdateClientScProfile` es/en. **Bug real pescado por Playwright**: la primera versión definía `profileFields`/`profileWithDefaults` DESPUÉS de `scPanelBox` y el form "Crear en SellerCloud" los llamaba durante el render → `ReferenceError: Cannot access before initialization` (TDZ de `const`) — se movió el bloque arriba del panel. Gotchas de la sesión: los archivos del repo son **CRLF** (un script Node que busca `\n` no matchea), y `innerText` de los `<th>` llega en MAYÚSCULAS por el `uppercase` de CSS. **Verificado de verdad**: 16 bloques de assert SQL en PG 18 desechable (schema + sync-v2 + fix-phones + 08-18 + 08-25 + 08-31 + 09-02 reales) + re-aplicación idempotente + preflight negativo; `tests/sc-customers-tests.mjs` 26 → **44** (BusinessName en el create, `customerUpdateBody` solo con valor / 0 / negativo / string numérico, `updateCustomer` un PUT con las 5 claves / no llama sin nada / PUT fallido con contexto, `addCustomerToGroup` path + body / grupo 0 corta antes / grupo inexistente lanza); sc-push y stock-refresh siguen en verde; **Playwright** contra el build real con Supabase y la Edge Function interceptadas (admin: compacta 6 / completa 13 columnas con valores de la ficha, sin scroll horizontal de la página, preferencia persistida al recargar, buscador por empresa y comentario; alta: opciones = vendedoras con datos, prellenado por vendedora y lista, comentario propio no se pisa, insert con la ficha completa, Edge Function create sobre la fila insertada; edición: prellenado, RPC solo con lo cambiado y en orden RPC → `update`, banner verde con #ID, tabla refleja sin recargar, sin cambios sin llamadas, sin vínculo aviso ámbar sin Edge Function, warning de SellerCloud en ámbar y se cierra, RPC fallida deja la edición abierta; crear desde la fila con defaults de la vendedora y RPC antes del create; vendedora: solo sus valores y 2 opciones por select, vista completa, sin input de SellerCloud ID; Vendedoras: grupo `#4 nombre`, PATCH con id+nombre, ID sin nombre no guarda, vaciar → nulls; móvil 390 px sin desborde) — **122/122**; la prueba móvil pescó además que el form de alta YA desbordaba la página a 390 px antes de esta tanda (sin la ficha, peor: 469 px) — la grilla `md:grid-cols-2` no tenía `grid-cols-1` en móvil y la etiqueta del toggle "Crear también en SellerCloud" con su pista era un `flex` sin `flex-wrap`, así que la columna única se dimensionaba al min-content; corregido con `grid-cols-1` + `flex-wrap`/`min-w-0`, y los controles de la ficha llevan `min-w-0` por el mismo motivo (selects con opciones largas); build de Vite limpio. **Segunda iteración del mismo día** (el usuario ya había corrido la migración y redesplegado la función; reportó: "cuando se expande la tabla no hay manera de scrollear a los lados para ver los otros campos"): con 13 columnas la tabla supera el ancho del `main` (`max-w-6xl`), el `overflow-x-auto` sí scrolleaba pero su barra horizontal quedaba al PIE de una tabla de cientos de filas — fuera de pantalla — y la rueda del mouse no desplaza a los lados. Arreglo (solo en vista completa; la compacta sigue igual): el cuadro de la tabla (`clients-scroll-box`, `scrollBoxRef`) tiene **alto acotado con scroll vertical propio** — `max-height: max(240px, calc(100vh - top - 56px))` donde `top` se mide en cada render con `getBoundingClientRect` (un `100vh - Xrem` fijo fallaba porque lo que hay arriba —form de alta, carga por Excel, avisos— cambia); así la barra horizontal queda siempre a la vista al fondo del cuadro; **encabezado sticky** (`thead sticky top-0` con sombra de 1 px porque con `border-collapse` el borde no acompaña) y **columna Nombre sticky a la izquierda** (`bg-surface` sólido); fila de pista "Desplazá la tabla a los lados…" con **botones ◀ ▶** (`scrollBy` 320 px suave); las filas desplegadas (ficha en edición, panel SellerCloud) van en un `div` sticky a la izquierda acotado al ancho del cuadro para no irse con el scroll; el pie con el conteo quedó FUERA del cuadro. i18n `scrollSidewaysHint`/`scrollLeft`/`scrollRight`. Verificado con 16 aserciones Playwright nuevas (150 filas de relleno: la tabla es más ancha que el cuadro, el borde inferior del cuadro queda en pantalla, alto acotado con `overflow-y: auto`, thead/th/td sticky, ▶ desplaza y Nombre sigue pegado a la izquierda, ◀ vuelve, encabezado fijo al scrollear dentro, pie a la vista, compacta sin alto acotado/sticky/flechas, y la ficha en edición queda dentro del cuadro visible con la tabla desplazada) — **138/138**, build limpio. **Pendiente: deploy del frontend.**

81. **⚙️ Sistema: filtro "clientes creados desde el catálogo", búsqueda y Exportar Excel** (2026-09-09, tercera tanda del día, a pedido del usuario: "en el apartado sistema se pueda filtrar la búsqueda de eventos por clientes creados desde el catálogo, además de tener la opción de poder descargar/exportar un excel con los datos que se filtren"). **Sin migración**: `get_system_logs` sigue filtrando solo severidad y origen, y alcanza — la pieza que faltaba era el DATO: hasta hoy el alta local de un cliente no dejaba rastro en `system_logs` (solo la Edge Function logueaba el resultado en SellerCloud, y ni siquiera `sellercloud_customers` estaba en el select de orígenes de la pestaña). **(1) Rastro**: `ClientsAdmin.jsx` emite `logEvent('info', 'clients', 'client_created', '<nombre> · <teléfono>', context)` tras cada insert exitoso — alta individual (`via: 'panel'`, `sellercloud_requested` = el toggle) y cada cliente NUEVO de la carga por Excel (`via: 'excel'`, `file`; los actualizados no) — con la ficha **por nombres** (price_list = label, vendedora, group = nombre del grupo, account_manager = nombre de la vendedora, salesman, comments, business_name, email, phone, client_id) porque es lo que se exporta tal cual, y `created_by` = email de la sesión (`supabase.auth.getSession()`, sin request: `log_event` no guarda identidad). Fire-and-forget, nunca frena el alta. **(2) Pestaña** (`SystemLogsAdmin.jsx`): chip **👤 Clientes creados desde el catálogo** (`PRESETS`: un chip = un source; comparte estado con el select de origen, `aria-pressed`, pista al lado), **búsqueda de texto** (`searchTerms`/`matchesTerms` sobre evento + mensaje + `JSON.stringify(context)` como UN campo — la regla de "todos los términos en el mismo campo" del resto del panel) sobre lo ya cargado, contador "visibles / cargados" y pista bajo "Cargar más"; las filas de `clients` muestran en línea vía · lista · vendedora · comentario · creado por; el select de origen suma `sellercloud_customers`, `clients`, `stock_refresh`, `manual_refresh`. **(3) Exportar Excel**: recorre TODAS las páginas de la RPC (`p_limit` 500 = el máximo que clampea, cursor `p_before`) hasta `EXPORT_MAX` 5,000 (aviso ámbar si se llegó) — a propósito no exporta "lo cargado": los datos que se filtren son todos los que cumplen el filtro, no los 100 en pantalla; aplica la búsqueda del lado del cliente; traduce `via` (formulario/Excel); membrete (título · Filtro con el nombre del chip · Búsqueda si hay · Generado con el conteo) + blanco + tabla con autofiltro reutilizando `buildPriceListWorkbook` vía los alias neutros `buildTableWorkbook`/`downloadTableExcel`; `systemLogsExcelRows({t, rows, labels, maxContextCols, stamp})` NUEVO en `utils/excel.js` **aplana el context**: unión de claves en orden de aparición → una columna cada una (etiqueta de `labels` si se conoce, si no la clave cruda), null → sin celda, booleano → Sí/No, objeto/array → JSON, y por encima de 40 claves el resto va junto en una última columna "Detalle"; archivo `zimaxx-logs-sistema[-origen]-<fecha>.xlsx`, botón deshabilitado sin filas, aviso rojo si el filtro no deja nada. i18n `systemPreset*`/`systemSearch*`/`systemExport*`/`systemCtx*`/`systemVia*` es/en. **Verificado de verdad**: `tests/system-logs-excel-tests.mjs` NUEVO (23 en Node: fijas + claves en orden y etiquetadas, null/booleano/anidado, claves de una sola fila, tope de columnas con "Detalle", membrete/blanco/encabezado/autofiltro `A5:P8`/anchos releyendo el workbook); 62 aserciones Playwright NUEVAS en la pestaña contra el build real con `get_system_logs` interceptada (620 logs sembrados, 250 de `clients`: 100 filas + Cargar más + sources nuevos; chip → RPC con `p_source` y severidad libre, `aria-pressed`, select reflejado, pista, resumen en línea; búsqueda "zimaxx box" recorta lo cargado con contador y Cargar más suma; export con chip + búsqueda → una página de 500, nombre con sufijo, hoja "Sistema", membrete con el nombre del chip y la búsqueda, columnas etiquetadas, TODAS las filas que cumplen (más que las 100 visibles), vía traducida, Sí/No, autofiltro; export sin filtros → 2 páginas de 500 con cursor y 620 filas; severidad sin filas deshabilita el botón, búsqueda sin match → aviso de export vacío que se cierra; 5,001 filas de `sync` → aviso de tope, 5,000 exactas en 10 páginas; admin común sigue sin la pestaña) + 6 en Clientes (el alta dispara `client_created` con mensaje, ficha por nombres, `client_id` = el que recibió la Edge Function, vía panel, SellerCloud pedido, `created_by`) — **Sistema 62/62, Clientes 144/144**, suites Node previas en verde, build limpio. Gotcha de la sesión: `XLSX.read` no repuebla `!cols` sin `cellStyles` — los anchos se comprueban en el workbook construido. **Pendiente: deploy del frontend** (sin migración ni redeploy de funciones). **Pendiente**: correr la migración en Supabase, `supabase functions deploy sellercloud-customers`, deploy del frontend. Nada corrió contra producción ni contra la API real (solo lecturas con `db query --linked`).

82. **Auditoría doc ↔ código ↔ producción + hotfix re-aplicado en `dev`** (2026-09-10, a pedido del usuario: "lee el .md… audita el código para revisar que todo coincida ya que ocurrió una situación y tuvimos que hacer unos cambios rápidos", y después "aplica el hotfix"). **Hallazgo principal**: el hotfix del 2026-09-09 (`bf08618`, rama `hotfix/desactivar-creacion-sc`, hoy = `main` = lo desplegado en Netlify) apagó el alta de clientes en SellerCloud por flag en cinco archivos, pero el merge `Merge branch 'main' into dev` (`7850597`, ya en `origin/dev`) resolvió los conflictos de `ClientsAdmin.jsx` y de este archivo quedándose con la versión de `dev` (el WIP de la ficha, `1b6b25d`): en `dev` no existía `SC_CREATE_ENABLED`, el toggle "Crear también en SellerCloud" nacía prendido, el panel ofrecía "Crear en SellerCloud"/"Crear de todos modos" y `createSc` llamaba a la Edge Function sin candado; sobrevivieron las keys i18n `scCreateDisabled`/`scPickToLinkOnly` (huérfanas), el `CREATE_ENABLED = false` de la función y la fila Clientes del README (que describía una UI inexistente en `dev`). Efecto si se hubiera deployado `dev` así: cada alta llamaba `create`, recibía 503 y mostraba "Cliente creado; no se pudo dar de alta en SellerCloud" + el aviso de desactivada (confuso, aunque no creaba nada allá). **Hoy se re-aplicó en `dev` el bloque completo del flag** (6 hunks sobre `ClientsAdmin.jsx`: flag + `EMPTY_CLIENT.scCreate`, guard en `createSc`, `scOn`, texto `scPickToLinkOnly`, panel sin botones de crear, toggle apagado/deshabilitado con aviso y tooltip) **y, a continuación, a pedido del usuario los dos flags quedaron en `true` en `dev`** ("en esta versión de dev estamos reparando la herramienta de crear clientes en SellerCloud, aquí no puede estar bloqueada la función"): el mecanismo existe en las dos ramas y solo cambia el valor — `main`/producción `false` (hotfix), `dev` `true` (rama de reparación). El resto del WIP (ficha, vista completa, ✉️, Sistema) queda intacto; los comentarios de los dos flags documentan el valor por rama y avisan que en un conflicto de merge hay que conservar el bloque y elegir el valor según la rama destino. **Qué decía el hotfix original** (texto perdido en el merge): se apaga por flag, sin borrar código, para que la rama de reparación no reconstruya nada; "🔍 Buscar en SellerCloud" → candidatos → "Vincular" sigue igual; para reactivar, los dos flags en `true` + deploy del frontend + `npx supabase functions deploy sellercloud-customers` (desde `zimaxx-store/`). **Estado real de producción verificado el 2026-09-10** (sin escribir nada: `npx supabase functions list`, `functions download` al scratchpad + `diff --strip-trailing-cr` — el repo es CRLF y sin ese flag "difiere" todo —, `db query --linked`, curl al bundle de Netlify): (a) frontend Netlify = build de `main` (candado activo, `scCreateDisabled` ×4 en el chunk de Clientes; los puntos 78-81 NO desplegados); (b) `sellercloud-customers` **v4 = versión del hotfix**: tenía el candado pero NO la ficha ni `action:'update'` (214 líneas de diferencia con el repo) — el deploy del hotfix (15:36) se hizo desde una rama sin la ficha y pisó el redeploy del WIP que el punto 80 daba por hecho; **a las 14:29 del 09-10 el usuario la redesplegó desde el árbol de trabajo → v5 = candado `CREATE_ENABLED = false` + ficha + `update`** (verificado bajándola: idéntica al repo salvo el valor del flag, que en `dev` pasó a `true` después) — o sea, producción quedó con función Y frontend candados y la ficha ya disponible para cuando se deploye el frontend; `sellercloud-push-order` v14 y `sellercloud-refresh-stock` v1 idénticas al repo (salvo los helpers de customers en `sellercloud.ts`, que el push no usa); `admin-create-vendedora-user` v5; **`superadmin-users` estaba SIN desplegar** (el hallazgo del 08-19 seguía vigente; el checklist que la marca desplegada el 08-05 había quedado desactualizado) y **se redesplegó el mismo 09-10 a pedido del usuario → v2**, sonda sin JWT `401` = viva (hasta entonces "Cambiar contraseña" y "+ Crear admin" del panel 🔐 estaban rotos en producción); (c) base: corridas TODAS las migraciones hasta la 09-09 — incluidas 08-24 (`sync_upsert_products` dice `35 days`), 08-26 (`apply_order_stock` existe), 09-02 ×3 y 09-04 (`update_order_status` ya es la de 3 parámetros con `p_override`) — y la 08-13 `dismiss-order-failures` la corrió el usuario hoy (hasta entonces el banner rojo de Pedidos quedaba vacío en silencio con 77 fallos sin recuperar detrás); una sola policy en forma vieja, `inventory_syncs / admin_read_inventory_syncs` (`is_admin()` sin `(select …)`, de la 09-04); (d) `system_logs` `sellercloud_customers`: 20 `create_ok` vs **95 `create_failed`** hasta el 09-09 15:15 — la "situación" que motivó el hotfix; 1046 de 3518 clientes con `sellercloud_id`; 12 de 16 vendedoras con grupo sembrado; 0 fichas cargadas y 0 auditorías `update_client_sc_profile` (esperado: el frontend de la ficha no está desplegado). **Desfasajes del doc detectados y NO corregidos en esta tanda** (quedan para la lista): el sistema de frescura de inventario del 2026-09-04 (`migration-2026-09-04-inventory-freshness.sql`, tablas `app_settings`/`inventory_syncs`, funciones `get_inventory_freshness`/`audit_freshness_override`/`inventory_sync_begin`/`inventory_sync_finish`/`refresh_stock_upsert`/`sa_set_stock_freshness`/`stock_freshness_minutes` + `update_order_status` con `p_override`, Edge Function `sellercloud-refresh-stock`, `InventoryFreshness.jsx`, `useInventoryFreshness.js`, `freshness.ts`, push-order v14) no está en este archivo — por eso la sección RPC documenta `update_order_status(uuid, text)` y la real lleva `p_override boolean`, y el encabezado habla de la v13 del push; "Estructura de archivos" lista 2 de 7 tests y le faltan `SystemLogsAdmin.jsx`, `ui.jsx`, `OutboxBanner.jsx`, `hooks/`, `quotePdf/search/systemLog/favorites`, `scripts/` y 4 de las 5 Edge Functions (ídem README §8); `schema.sql` no incluye nada posterior al 08-18 salvo la 08-31; el checklist marca pendientes 08-24/08-25/08-26 ya corridas; el punto 81 cierra con "pendiente: correr la migración y redesplegar la función", contradiciendo al 80. **Lo que SÍ coincide** (verificado línea por línea): migración 09-09, Edge Function del repo, `sellercloud.ts`, `ClientsAdmin.jsx`, `VendedoresAdmin.jsx`, `SystemLogsAdmin.jsx`, `AuditLogAdmin.jsx` y `excel.js` hacen lo que describen los puntos 78-81; suites Node en verde (sc-customers 44/44, system-logs-excel 23, price-list-excel 40, outbox 26, backfill 27, sc-push OK, stock-refresh 56); build limpio; i18n 563/563 sin faltantes ni duplicadas. **Verificado tras re-aplicar**: build de Vite limpio + **35/35 aserciones Playwright** contra el build real (vite preview) con Supabase y la Edge Function interceptadas, receta de siempre (sesión falsa en `sb-<ref>-auth-token`, `page.route` sobre todo el host): admin/es — toggle deshabilitado y apagado, aviso en lugar de la pista y en el tooltip, un solo campo Nombre y ningún Apellido, click forzado no lo prende, el alta hace UN insert con el nombre entero y el teléfono limpio, CERO llamadas a la función, sin panel tras el alta, `client_created` con `sellercloud_requested: false`, la fila nueva aparece; panel — sin candidatos muestra "Sin coincidencias" + el aviso y ningún botón de crear, con candidatos texto solo-vincular (sin "o crealo"), dos "Vincular", ninguno de crear, Vincular llama `link` con el ID elegido y la fila lo muestra, dos búsquedas y ninguna `create`; vendedora — mismo candado; en — aviso y texto traducidos, sin "Create"; sin errores de página. Con los flags en `true` (estado final de `dev`), segunda suite **25/25** (`flag-on.mjs`, misma receta, con la Edge Function falsa aceptando `create`): toggle habilitado y prendido con la pista normal, nombre PARTIDO con campo Apellido, el alta inserta `name` = nombre + apellido y llama `create` con first/last, el id local recién creado y sin force, el panel confirma "Creado y vinculado (#id)", `client_created` con `sellercloud_requested: true`; desde la fila sin coincidencias aparece "Crear en SellerCloud", el form prellena partiendo el nombre (última palabra → apellido), la ficha se guarda por `update_client_sc_profile` ANTES del create, el create va con force y la fila muestra el ID; con coincidencias el texto largo ("…o crealo") + dos "Vincular" + "Crear en SellerCloud"; en inglés igual; sin errores de página. Gotcha de la prueba: `filter({ has })` no acepta un locator ya anclado (hay que pasar uno relativo) y el primer `<p>` del panel es el encabezado, no la introducción. **Pendiente**: deploy del frontend desde `dev` — **OJO: con `SC_CREATE_ENABLED = true` reactiva el alta en producción para todas las vendedoras aunque la reparación no haya terminado**; y cualquier redeploy de `sellercloud-customers` desde `dev` abre el candado del servidor (`CREATE_ENABLED = true`) incluso para el frontend de `main`, que quedaría protegido solo por su flag — confirmar el valor de los dos flags antes de cada deploy (la v5 viva ya tiene la ficha y `update`, así que hoy NO hace falta redesplegarla); envolver la policy de `inventory_syncs`; documentar la frescura del 09-04 y actualizar estructura/checklist. Nada de esta tanda tocó producción (solo lecturas) y nada está commiteado.

83. **El teléfono va debajo del nombre en la tabla de Clientes** (2026-09-10, segunda tanda del día, a pedido del usuario: "vamos a arreglar diseño del sistema, en la tabla de clientes quita la columna de teléfono y haz que salga debajo del nombre"). Solo frontend (`ClientsAdmin.jsx`), sin migración: la celda Nombre pasa a apilar el nombre (font-medium) y debajo el teléfono en mono chico (`text-primary/60`, `whitespace-nowrap`), con la flechita ✉️ del correo (punto 79) pegada al teléfono — sigue siendo solo de la compacta, en la completa el correo tiene columna propia; la columna Tel desaparece de las dos vistas y `colCount` pasa de 6/13 a **5/12** (compacta: Nombre, SellerCloud, Lista, Vendedora, acciones; completa: Nombre, Empresa, Email, SellerCloud, Grupo, Account manager, Salesman, Comentarios, Lista, Vendedora, Alta, acciones), así que las filas desplegadas (correo, panel SellerCloud, ficha en edición) siguen alineadas. Al Editar, los tres inputs —nombre, teléfono (`inputmode="tel"`) y correo— van apilados en esa misma celda con ancho uniforme (`w-44`); Enter/Escape y la validación no cambian. En la vista completa la columna sticky Nombre lleva ahora también el teléfono (misma celda, fondo sólido). Docs: sección "Edición de nombre/teléfono…" de este archivo y README (Clientes: edición, vista completa y párrafo nuevo "El teléfono va debajo del nombre"). **Verificado**: build de Vite limpio + **25/25 aserciones Playwright** contra el build real con Supabase interceptado (compacta a 1280 px: 5 encabezados sin "Tel", nombre y teléfono en la primera celda con el teléfono geométricamente debajo, la 2ª celda ya es SellerCloud, 5 celdas por fila, flechita ✉️ en la primera celda que abre la fila del correo con `colspan=5`, sin scroll horizontal de la página; Editar apila 3 inputs prellenados (2º tel, 3º email), Enter guarda por `update_client_info` con el teléfono nuevo y la celda lo refleja debajo del nombre; completa a 1400 px: 12 encabezados con Email y sin Tel, 12 celdas, sin flechita, 3 inputs al editar, fila de la ficha con `colspan=12`, sin scroll horizontal de la página; móvil 390 px: nombre y teléfono juntos y la página no desborda; sin errores de página). Gotcha repetido, ya anotado en la memoria del proyecto: al entrar en edición la fila deja de matchear por texto — localizarla por índice antes de tocar "Editar". **Pendiente: deploy del frontend** (el mismo de siempre).

84. **"Pedidos que no se registraron" como cuadro expandible, con tipo y contenido por fallo + diagnóstico de los 77 fallos** (2026-09-10, tercera tanda del día, a pedido del usuario: "hay un temita con la recuperación de los pedidos, intuyo que tiene que ver con alguna de las funciones que no estaban activas, salieron muchos pedidos que no llegaron y salen para recuperar; quiero que eso salga como un cuadro expandible, y que la información de los pedidos también sea expandible para ver qué contiene cada uno, que diga si es un pedido o una cotización"). **Diagnóstico primero (producción, solo lectura)**: los 77 fallos pendientes NO tienen que ver con funciones apagadas — son rechazos de `create_order` por la regla require-price del 2026-08-06 ("productos sin precio en la lista del cliente: SKU…", 75 de 77; más 1 `token inválido` del 08-12 y 1 cotización "ningún ítem válido" del 08-26), acumulados entre el 2026-08-12 y hoy, que se hicieron visibles DE GOLPE al correr `dismiss-order-failures` (hasta entonces el banner estaba mudo, punto 82). De las 145 líneas-SKU que nombran los motivos: los 145 productos existen, 75 están activos, **64 tienen precio HOY en la lista del cliente** (al momento del pedido no lo tenían: la carga semanal de precios los había sacado o todavía no los había repuesto) y **81 siguen sin precio** (la mayoría inactivos: salieron de la lista y de la publicación). O sea: el cliente tenía en el carrito —que persiste en `localStorage`— productos que perdieron el precio en su lista entre que los agregó y que envió, y un pedido real con UNA línea sin precio se rechaza entero (decisión del punto 52). "Recuperar" sigue siendo la salida: crea la cotización con los precios vigentes y deja `price: null` en las líneas que hoy no lo tienen. **UI (`OrdersAdmin.jsx`, sin migración)**: el bloque rojo pasa a ser un **cuadro plegado por defecto** con el conteo total y por tipo ("3 Pedidos · 1 Cotización") y un botón "Ver detalle ▾ / Ocultar detalle ▴" (`aria-expanded`); dentro, cada fallo lleva el **badge Pedido/Cotización** (el mismo de la bandeja, por `order_failures.kind`) junto al cliente, y un botón **"Ver contenido ▾"** que despliega una tabla Producto · Cantidad (y Precio unit. · Subtotal solo si el payload traía precio) con pie "N líneas · M unidades [· Total]". El payload puede ser completo (`{id, sku, name, qty, price}`, del drawer o del outbox expirado) o **slim** (`{id, qty, flash}`, el outbox en el wire): para el slim se resuelven sku/nombre con `products` (`select id, sku, name, upc`, de a 100 ids por URL — el `in.(...)` viaja en la query string), un producto borrado dice "Producto no encontrado" con su cantidad, y **las líneas que nombra el motivo "sin precio en la lista" van en rojo con etiqueta** (se parsean los SKU tras los dos puntos). Nunca recalcula precios: si el payload no traía, no se inventa. Sin ítems (token inválido) muestra el aviso y solo ofrece Descartar, como antes. i18n `quotes`/`failedOrdersShow`/`failedOrdersHide`/`failureItemsShow`/`failureItemsHide`/`failureItemsNone`/`failureMissingPrice`/`failureUnknownProduct`/`failureUnits` es/en. **Verificado**: build limpio, i18n 572/572 sin faltantes, **28/28 aserciones Playwright** contra el build real con Supabase interceptado (4 fallos sembrados: slim con SKU sin precio, cotización con producto borrado, completo con precio, token inválido — plegado por defecto sin Recuperar a la vista, conteo por tipo, `aria-expanded`, 4 filas al abrir con 3 badges Pedido + 1 Cotización, 3 Recuperar + 1 Descartar, el slim resuelve nombres con UNA consulta de 2 ids y marca solo la línea del motivo, sin columnas de precio y pie sin total; el completo no consulta y muestra $20.50/$102.50 con Total; producto borrado; sin ítems; plegar; ninguna RPC de recuperar/descartar disparada; inglés). **Pendiente: deploy del frontend.** Qué hacer con los 77: revisarlos desde el cuadro nuevo — los que hoy sí tienen precio se recuperan como cotización y se convierten; los que siguen sin precio necesitan que se cargue el precio en la lista (o se descartan si el producto ya no se vende).

85. **Fecha visible por fallo + "Descartar" en todos los pedidos que no se registraron** (2026-09-11, a pedido del usuario: "me gustaría que los pedidos que no se registraron tengan la fecha de cuando son, además de la opción de descartarlos por si no se quiere recuperar"). **Qué había**: la fecha SÍ estaba, pero en gris chico delante del motivo, con el `toLocaleString()` del navegador ("9/8/2026, 3:00:00 PM") y pegada a un motivo largo ("productos sin precio en la lista del cliente: SKU, SKU…") — en una lista de 77 filas no se veía; y "Descartar" solo aparecía cuando "Recuperar" no podía (sin cliente o sin ítems, punto 60), así que un fallo recuperable que el negocio no quería recuperar se quedaba en el cuadro para siempre. **UI (`OrdersAdmin.jsx` + i18n, sin migración)**: (1) cada fallo lleva una **línea propia de fecha** (`data-testid="failure-date"`), en el idioma del panel (`es-VE`/`en-US`, igual que Clientes) con día de semana, fecha y hora, más el día relativo por calendario local — "hoy", "ayer", "hace N días" (`failureAgo`, por día calendario y no por 24 h: algo de anoche a las 23:00 es "ayer"); el motivo y las líneas pasan a la línea de abajo. (2) El **resumen plegado** agrega el rango de fechas ("📅 del 12 ago. 2026 al 11 sept. 2026", o un solo día si todos son del mismo; `data-testid="failures-range"`) para saber de cuándo son sin abrir la lista. (3) **"Descartar" en todos los fallos** (`data-testid="failure-dismiss"`), a la derecha de "Recuperar" en los recuperables: en esos abre un **modal de confirmación** (`dismiss-confirm`, misma receta que el de cambio de estado, borde rojo) que dice de quién y de cuándo es ("El intento de {client} del {date} dejará de aparecer en esta lista y ya no se podrá recuperar desde el panel. La fila no se borra y la acción queda en la auditoría.") y resume tipo · líneas · motivo; Cancelar cierra sin llamar a nada, Descartar llama a `dismiss_order_failure` y recarga. Sin cliente o sin ítems sigue siendo directo, sin modal (no hay nada que perder). El error de la RPC se muestra inline en la fila, como antes. (4) Tope del cuadro **50 → 200** (`FAILURES_CAP`): con 77 pendientes el encabezado decía "50" y el rango hubiera mentido; al tope, el resumen agrega "Se muestran los 200 más recientes". (5) `failedOrdersBody` explica los dos botones. **La RPC no cambió**: `dismiss_order_failure` (08-13, corrida en producción el 09-10) nunca exigió que el fallo fuera irrecuperable — mismos permisos que recuperar (admin cualquiera, vendedora solo sus clientes), audita en `admin_audit_log` con `reason`/`kind`/`token_hint`. **Límite conocido**: `created_at` es cuando `create_order` rechazó el intento; para los del outbox expirado (`report_outbox_expired`, 09-02) es cuando el teléfono lo reportó, ≥24 h después de que el cliente tocó Enviar — la RPC no viaja con la hora original y cambiarlo pediría migración + deploy coordinado, fuera de esta tanda. i18n nuevas es/en: `failedOrdersRange`, `failedOrdersCapped`, `failureAgoToday`, `failureAgoYesterday`, `failureAgoDays`, `dismissConfirmTitle`, `dismissConfirmBody`. **Verificado**: build limpio; i18n 579/579 sin faltantes ni duplicadas; **40/40 aserciones Playwright** contra el build real (`vite preview` + Supabase interceptado, 4 fallos sembrados con fechas relativas a hoy: hace 3 días, ayer, hoy, hace 30 días): rango en el resumen (es: "del 12 ago. 2026 al 11 sept. 2026"; en: "from Aug 12, 2026 to Sep 11, 2026"), 4 líneas de fecha con año y hora y "hace 3 días"/"ayer"/"hoy"/"hace 30 días", motivo en su propia línea, 3 Recuperar + 4 Descartar; Descartar en un recuperable abre el modal con cliente, tipo, líneas y motivo sin llamar a la RPC; Cancelar cierra y quedan 4; confirmar llama UNA vez a `dismiss_order_failure` con el id correcto, la fila se va, el encabezado baja a (3) y el resumen a "2 Pedidos · 1 Cotización"; el de token inválido se descarta directo sin modal; una RPC que falla deja el mensaje del servidor en la fila y la fila se queda; nunca se llamó a recuperar; inglés ("Dismiss this attempt?", "Cliente Uno's attempt", "3 days ago", "today"); sin errores de página. La suite del punto 84 sigue 27/28: la única aserción que falla es "1 fila solo con Descartar", que ahora es 4 por diseño. **Pendiente: deploy del frontend** (junto con el 09-10). Nada tocó producción.

86. **Correcciones al alta de clientes en SellerCloud: mayorista y compañía explícitos + placeholder de Empresa** (2026-09-11, segunda tanda del día, a pedido del usuario: "el campo de crear cliente, en la entrada de business name (Empresa) en el placeholder no debe tener '(opcional)'; cuando se creen los clientes deben marcarse con el campo isWholesale: true, y además con el company que sea Zimaxx"). **(1) Placeholder**: el input Empresa de la ficha (`ClientsAdmin.jsx`, alta y edición) usa `t('businessName')`; la key `businessNameOptional` se borró de es/en (era su único uso; i18n 578/578). **(2) Mayorista + compañía**: `CustomerFields` gana `isWholesale?: boolean | null` y `companyId?: number | null`, y `customerUpdateBody` los traduce a `IsWholesale` (solo booleano explícito) y `CompanyId` (solo entero positivo) — los dos existen en el `UpdateCustomerRequest` del Swagger real (`/rest/swagger/docs/v1`, guardado de la sesión del 09-02). `pushProfile` en `sellercloud-customers/index.ts` los manda SIEMPRE en el mismo PUT de la ficha con `isWholesale: true, companyId: cfg.companyId` (`SELLERCLOUD_COMPANY_ID`, la misma compañía que llevan las órdenes) — vale para `action:'create'` (PUT tras el create) y para `action:'update'`. El create no cambió (sigue `CustomerType: 1` + `CompanyID`): el Swagger es ambiguo con ese enum (0 = WholeSale en el CreateCustomerRequest, 1 = Wholesale en el filtro del GET, punto 76) y el PUT deja las dos cosas fijas sin depender de él; si un customer creado seguía saliendo Retail, esto lo corrige en el segundo paso. El aviso de PUT fallido nombra ahora también mayorista/compañía. **Supuesto**: la compañía "Zimaxx" es la del secret `SELLERCLOUD_COMPANY_ID` (el listado de n8n usa `companyIds=172`); si la cuenta tuviera otra compañía, se cambia el secret, no el código. El backfill (`setCustomerPhone`) no se tocó: no marca nada en customers ya existentes. **Verificado**: `node tests/sc-customers-tests.mjs` **53/53** (nuevas: IsWholesale true/false/null/ausente, CompanyId 8/0/null/NaN, y el PUT de la ficha completa con 7 claves incluidas `IsWholesale` y `CompanyId`); build limpio; i18n 578/578 sin faltantes. **Desplegado el mismo día por el usuario**: `npx supabase functions deploy sellercloud-customers` desde `zimaxx-store/` (con `&&`; con `&` simple el `cd` va a segundo plano y falla con "Cannot find project ref") → **v6** (`functions list`: `updated_at` 2026-09-11; sonda `POST /functions/v1/sellercloud-customers` sin JWT → 401 = viva), subiendo `index.ts` + `sellercloud.ts`. **Consecuencia**: la v6 lleva `CREATE_ENABLED = true` (valor de `dev`), así que **el candado del servidor del hotfix 09-09 quedó abierto**; hoy producción no crea clientes en SellerCloud solo porque el frontend desplegado sigue siendo `main` con `SC_CREATE_ENABLED = false` (punto 82). **Pendiente: deploy del frontend** — al hacerlo desde `dev` (`SC_CREATE_ENABLED = true`) el alta vuelve a estar activa para todas las vendedoras; si eso no se quiere todavía, poner el flag del frontend en `false` antes de ese deploy.

87. **Rango "Histórico" en 📈 Métricas: desde el primer pedido hasta hoy** (2026-09-11, tercera tanda del día, a pedido del usuario: "que el rango de las métricas mantenga las opciones que ya se tienen y que además se agregue un histórico, que sea las métricas desde que se hizo la primera orden hasta la actual"). **Por qué hace falta migración**: la RPC `sa_metrics_overview` clampea `p_days` a `[1, 365]` y arma un bucket por día, así que "desde el primer pedido" no se podía pedir desde el frontend. **`migration-2026-09-11-sa-metrics-historic.sql`** reemplaza la función con la **misma firma** `(p_days int default 14)`: `p_days > 0` es exactamente lo de antes; **`p_days = 0` = Histórico** — `v_from` = 00:00 del día del primer pedido real (`min(orders.created_at)` con la misma exclusión de cuentas de prueba que todo lo demás, así un pedido de "Pruebas" de hace un año no estira la ventana; sin pedidos → ventana de hoy), `v_to` = ahora, sin clamp, `days` = días calendario redondeados hacia arriba. `period` gana `historic` (bool), `since` (created_at del primer pedido, null si no hay) y `bucket` (`'day'` | `'week'`). **Serie por semana cuando el rango pasa los 120 días** (`date_trunc('week')`, `dia` = lunes): 365+ barras diarias no se leen y el `generate_series` se queda chico aunque el histórico tenga años; el total del período sigue siendo exactamente la suma de la serie. Efecto lateral asumido: un `p_days` a mano > 120 (99999 → 365) también sale semanal. Grants iguales (`authenticated` sí, `anon` no). **Frontend (`MetricsAdmin.jsx` + i18n)**: `RANGES = [7, 14, 30]` intacto, default 14; botón **"Histórico"** (`HISTORIC = 0`, `data-testid="metrics-range-historic"`, `aria-pressed`) a la derecha de los tres; la nota bajo el gráfico pasa de "el primer día es parcial" a **"Histórico: desde el primer pedido (23 feb. 2026) hasta hoy, 201 días."** más "Con un rango tan largo las barras son por semana." cuando `bucket = 'week'`; el título del gráfico dice "Monto por semana" en ese caso; el Excel sale como `zimaxx-metricas-historico-…`. **Detección de RPC vieja**: con la función anterior un `p_days = 0` se clampea a 1 y responde SIN `period.historic`; el frontend lo detecta (`days === HISTORIC && payload.period.historic !== true`), vacía la pantalla y muestra `metricsHistoricMissing` ("Falta correr migration-2026-09-11-sa-metrics-historic.sql…") en vez de pintar el último día como si fuera el histórico. i18n nuevas es/en: `metricsHistoric`, `metricsTrendWeek`, `metricsHistoricNote`, `metricsHistoricWeekly`, `metricsHistoricMissing` (583/583). **Verificado**: (a) **la migración, contra un PostgreSQL 18 desechable** (esquema mínimo + stubs de `is_superadmin()` por GUC y los helpers reales de cuentas de prueba; corrida DOS veces = idempotente): T1 histórico con un pedido de prueba de 400 días y el primer real de 200 → `historic = true`, `bucket = 'week'`, `since` = el real, `from` = su 00:00, `days` = 201, pedidos 4 / monto 250 / cotizaciones 1 / cancelados 1, la serie suma 250 en 29 barras que arrancan en lunes, `por_vendedora` con Ana + sin vendedora y sin la de prueba, `excluidas` la lista; T2 `(14)` intacto (`historic = false`, `bucket = 'day'`, 15 puntos, `since` null); T3 `null` → 14; T4 `99999` → 365 semanal (~53 barras); T5 `-5` → 1 día, 2 puntos; T6 sin superadmin → `not authorized`; T7 sin pedidos → `historic = true`, `since` null, `days` 1, ceros, ticket null, 1 punto; T8 grants. (b) **30/30 Playwright** contra el build real con la RPC interceptada: arranca pidiendo 14, siguen los tres botones, "Histórico" sin seleccionar, nota de 14 días y "Monto por día"; click → `p_days = 0`, nota histórica con fecha y 201 días + aviso semanal, sin la frase del día parcial, `aria-pressed`, "Monto por semana", KPIs del histórico, 29 barras, pie de la tabla; volver a 14 restaura todo; **RPC vieja** → 14 días andan igual y Histórico muestra el aviso de la migración SIN números; inglés ("All time", "All time: from the first order (Feb 23, 2026) to today, 201 days.", "Amount per week"); sin errores de página. (c) Build limpio. **Gotchas de la prueba**: `Start-Process pg_ctl start -Wait` cuelga (espera al árbol de hijos; la espera es `pg_isready`), `money()` no pone separador de miles y el pie `uppercase` obliga a comparar con `textContent`. `schema.sql` NO se tocó (sigue con la versión del 08-18 de esta función, como el resto de lo posterior). **Migración corrida en producción el mismo día** por el usuario con `npx supabase db query --linked -f supabase/migration-2026-09-11-sa-metrics-historic.sql` desde `zimaxx-store/` (el `!` ya estaba parado ahí: `cd zimaxx-store` falló con "No such file or directory" — la carpeta del `!` sigue a la del Bash tool). Verificado en `pg_proc` (solo lectura): `sa_metrics_overview(p_days integer DEFAULT 14)` viva tiene `v_historic` y `date_trunc(v_bucket`, `authenticated` ejecuta y `anon` no. Dato de producción: el primer pedido real es del **2026-07-27** (718 pedidos no cancelados hasta hoy), o sea que el histórico arranca con 47 días y sale por día; pasa a semanal solo del día 121 en adelante. **Pendiente: deploy del frontend** (el viejo ignora las claves nuevas de `period`).

88. **Preparación del deploy de lo pendiente del 09-08 al 09-11 + el alta en SellerCloud vuelve a quedar bloqueada en las dos ramas** (2026-09-14, a pedido del usuario: "quiero desplegar los cambios que tenemos hechos por ciertas correcciones pero también quiero volver a bloquear la función de creación de clientes a SellerCloud"; a mitad de la tanda: "no hagas el despliegue, de eso me encargo yo"). **Punto de partida**: `dev` = `7697416` + el árbol de trabajo sin commitear del 09-10/09-11 (cuadro de fallos con fecha/contenido/Descartar, Histórico en Métricas, mayorista+compañía, docs, `migration-2026-09-11-sa-metrics-historic.sql`), con los dos flags en `true`; `main` = `bf08618` (hotfix, `false`) = lo que sirve Netlify; `sellercloud-customers` viva = **v6 con `CREATE_ENABLED = true`** (candado del servidor abierto desde el 09-11); todas las migraciones corridas, incluida la 09-11. Como `main` es ancestro de `dev`, el merge es fast-forward: deployar `dev` tal cual habría reactivado el alta para todas las vendedoras. **(1) Flags a `false` en `dev`** (hecho): `SC_CREATE_ENABLED` (`ClientsAdmin.jsx`) y `CREATE_ENABLED` (`sellercloud-customers/index.ts`), con el comentario reescrito — **se deroga el "valor por rama" del 09-10** (`dev` en `true`): `main` y `dev` llevan el mismo valor y un merge no puede perderlo. Para retomar la reparación en `dev`: los dos en `true` sin deployar. Para reactivar el alta en producción: los dos en `true` + deploy del frontend + redeploy de la función. **(2) Edge Function** (hecho — corrió antes de que llegara el "no hagas el despliegue"): `npx supabase functions deploy sellercloud-customers` desde `zimaxx-store/` → **v7** (`functions list`: `updated_at` 2026-09-14 12:24 UTC); bajada con `functions download` al scratchpad y diffeada con `--strip-trailing-cr`: `index.ts` y `sellercloud.ts` **idénticos al repo**, `CREATE_ENABLED = false` — `create` responde 503 otra vez; `search`/`link`/`update` siguen, y el PUT de la ficha con `IsWholesale`/`CompanyId` (punto 86) queda vivo para `update` y listo para cuando se reactive el alta. Con esto producción tiene el candado cerrado en los dos lados aunque el frontend siga siendo `bf08618`. **(3) Frontend — lo hace el usuario, nada commiteado**: commit del árbol de trabajo de `dev` (features del 09-10/09-11 + flags + docs + la migración), push, `main` fast-forward a `dev` (`git merge --ff-only dev` parado en `main`), push → Netlify construye `main` (base directory `zimaxx-store`). Después conviene bajar `index.html` del sitio, seguir el chunk `index-*.js` hasta el de Clientes y confirmar `scCreateDisabled` (candado activo) — los hashes de Vite pueden no coincidir con el build local, se buscan marcadores. **Verificado en el árbol de trabajo**: build de Vite limpio; `node tests/sc-customers-tests.mjs` **53/53**; i18n **583/583** es/en sin faltantes ni duplicadas; **35/35 Playwright** con la suite del candado apagado del 09-10 (`hotfix.mjs`, contra `vite preview` del `dist/` real con Supabase y la función interceptadas: toggle deshabilitado y apagado con aviso y tooltip, un solo campo Nombre, el alta hace UN insert local y CERO llamadas a la función, `client_created` con `sellercloud_requested: false`, panel solo buscar/vincular sin botones de crear con y sin candidatos, Vincular llama `link` con el ID elegido, vendedora con el mismo candado, inglés, sin errores de página); en `dist/`, el chunk de Clientes trae `scCreateDisabled` ×4 y `scPickToLinkOnly`, sin `businessNameOptional`; el de Métricas trae `metrics-range-historic` y el de Pedidos `failure-dismiss`. **Qué queda vivo en producción cuando el usuario haga ese push** (puntos 78-87): Excel de la lista de precios en el catálogo y en Precios; ✉️ correo visible/editable en Clientes; ficha SellerCloud (empresa, grupo, account manager, salesman, comentarios) en alta y edición + vista completa con scroll propio; ⚙️ Sistema con filtro de clientes creados, búsqueda y Excel; teléfono debajo del nombre; cuadro plegable de "Pedidos que no se registraron" con tipo, contenido, fecha y Descartar en todos; el PUT de la ficha con mayorista+compañía (solo vía "Actualizar ficha" mientras `create` esté bloqueado); rango Histórico en 📈 Métricas (la RPC ya está corrida). Los 77 fallos del punto 84 se podrán revisar desde el cuadro nuevo. Lo que NO cambia: buscar y vincular clientes con SellerCloud siguen andando; el push de órdenes no se tocó (v14). **Segunda tanda (misma tarde)**: el usuario hizo el commit y el push (`7924120`, `main` = `dev` = `origin`); bundle de Netlify verificado: `index-BmDRxyVU.js` (mismo hash que el build local), chunk de Clientes con `scCreateDisabled` ×4 y sin `businessNameOptional`, `metrics-range-historic` en Métricas, `failure-dismiss` en Pedidos → los puntos 78-87 están vivos con el alta bloqueada en los dos lados (frontend + v7). Después pidió "y ahora sigamos trabajando el dev con la función en true": los dos flags volvieron a `true` en el árbol de trabajo de `dev` (sin commitear; el usuario commitea cuando quiera), con el comentario de VALOR POR RAMA reescrito — `main` `false`, `dev` `true` — y la regla explícita: `main` es ancestro de `dev` (el merge es fast-forward) y `functions deploy` sube el árbol de trabajo, así que **antes de cada deploy desde `dev` se ponen los dos en `false`, se deploya, y se vuelve a `true`**. Verificado: build limpio + **25/25 Playwright** con la suite del flujo habilitado del 09-10 (`flag-on.mjs`: toggle habilitado y prendido con la pista normal, nombre partido con Apellido, el alta inserta nombre + apellido y llama `create` con first/last sin force, el panel confirma "Creado y vinculado", `client_created` con `sellercloud_requested: true`, "Crear en SellerCloud" desde la fila con la ficha guardada por RPC antes del create, texto largo + dos Vincular + Crear con candidatos, inglés, sin errores de página). Idea no pedida, anotada para cuando se quiera: mover el valor del candado a configuración (`VITE_SC_CREATE_ENABLED` en Netlify + un secret en Supabase) para que el código sea igual en las dos ramas y ningún deploy pueda cambiarlo por accidente.

89. **Dirección del cliente para SellerCloud: se guarda, viaja al crear el customer, se reintenta desde la fila y se puede cargar desde un pedido rechazado** (2026-09-14, tercera tanda del día, a pedido del usuario: "es muy importante que el cliente se guarde con el address correcto, y que ese campo no llegue vacío"; **la sesión se cortó por una falla eléctrica** con el código completo pero sin verificar ni documentar — ver "Cómo se retomó" al final). **Por qué**: `POST /Customers` NO acepta dirección; se carga aparte con `PUT /Customers/{id}/Addresses` y hasta hoy no se mandaba ni se guardaba en ningún lado. El push de órdenes lee la dirección del customer en SellerCloud y rebota si `Addresses` viene vacía: **6 de los 9 pushes fallidos entre el 07 y el 14 de septiembre** fueron por eso (dos de esos clientes se habían creado desde este catálogo). **Decisiones del usuario**: UNA dirección por cliente (envío = facturación); estado obligatorio para todos los países (en Venezuela también existe: Miranda, Zulia…); obligatoria solo cuando el cliente se crea en SellerCloud (un cliente local o de cotización puede no tenerla); y un flujo de n8n puede traer las direcciones de los customers que ya existen allá. **Contrato de la API (Swagger real)**: `PUT /Customers/{id}/Addresses {Addresses: UserAddressDto[]}` — cada dirección con `ID` (0 = nueva), `AddressSource` (0 = LocalSite), `AddressStatus` (2 = Confirmed), `IsShippingAddress`, `IsBillingAddress`, `Country`, `City`, `ZipCode`, `Address` requeridos + `Address2`, `State`, `Region`, `ContactName`, `CompanyName`, `Phone` opcionales; `GET /Customers/{id}` devuelve `Addresses[]` con `ID`. El Swagger no dice si el PUT reemplaza o mezcla la lista, así que se manda la lista COMPLETA leída del customer con la nuestra adelante (si reemplaza, nada se pierde; si mezcla, las existentes viajan con su ID) y después se relee el customer y se busca la nuestra — por ID si se conoce, si no por calle + código postal normalizados (un reintento no duplica); si no aparece es ERROR, porque la orden rebotaría igual. **Migración `migration-2026-09-14-client-address.sql`** (aditiva, preflight, idempotente; **SIN CORRER en producción** — va ANTES del deploy del frontend y del redeploy de las dos funciones): `clients.address_line1/address_line2/address_city/address_state/address_zip/address_country` (país ISO-2 en mayúsculas, check `clients_address_country_iso2`) + `sc_address_id` (UserAddressDto.ID una vez verificado), `sc_address_synced_at` (cuándo se confirmó releyendo; null con dirección y vínculo = pendiente de mandar), `sc_address_error` (último fallo; null = ok). RPC **`update_client_address(p_client_id, p_line1, p_line2, p_city, p_state, p_zip, p_country) → jsonb`**: el valor NUEVO completo; regla **"COMPLETA (calle, ciudad, estado, código postal, país) o toda vacía, nunca a medias"** (una a medias es justo lo que SellerCloud rechaza); no deja borrarla si `sc_address_id` ya está ("se puede corregir, no borrar"); zip de 3 a 12; estado de US en mayúsculas si son dos letras; admin cualquiera, vendedora sus clientes; audita `'update_client_address'` con `from_x/to_x` SOLO de lo cambiado + `changes[]`; un cambio deja `sc_address_synced_at` en null; no-op sin fila; devuelve `{ok, changed, changes, complete}`. RPC **`mark_client_sc_address(p_client_id, p_sc_address_id, p_error) → jsonb`**: la llama la Edge Function con el JWT del caller tras el PUT + relectura — éxito → id + `synced_at = now()` + error null + audita `'sync_client_address'` con la dirección completa y el `sellercloud_id`; fallo → `sc_address_error` (sin auditar); `authenticated`. **`sync_upsert_clients(p_rows)`** (misma firma, n8n → acá) acepta por fila, OPCIONAL, `address/address2/city/state/zip/country/sc_address_id`: si trae calle + ciudad + código postal + país reconocido (el estado puede faltar en un customer viejo) la guarda COMO SINCRONIZADA (`synced_at = now()`, error null) y pisa lo local — para un customer que ya existe allá SellerCloud es la fuente de verdad; si no, descarta LA DIRECCIÓN y no la fila (contadores nuevos `addresses` / `addresses_skipped`); un payload sin esas claves no toca la dirección guardada; la adopción por teléfono también la trae. Helper `sync_country_iso2(text)`: ISO-2 directo o nombre en es/en de los 24 países del negocio (`service_role`). **Contrato para n8n**: de `Addresses[]` del customer mandar la marcada `IsShippingAddress` (o la única): `Address → address`, `Address2 → address2`, `City → city`, `State → state`, `ZipCode → zip`, `Country → country`, `ID → sc_address_id`. **`sellercloud.ts`**: `CustomerAddress`, `addressMissing` (calle/ciudad/estado/código postal/país), `addressDto` (envío Y facturación, LocalSite, Confirmed, país en mayúsculas, las 9 claves requeridas), `findAddress` (por ID, si no por calle+zip normalizados), `addressIdOf`, `customerAddressesBody` (lista completa con la nuestra adelante; si ya está, se actualiza EN SU LUGAR con su ID), `setCustomerAddress` (GET → PUT → GET; lanza si está incompleta, si el PUT falla o si al releer no aparece); `addressesOf` marca el error "Addresses vino vacía" con `code = 'customer_no_address'`. **Edge Function `sellercloud-customers`**: `loadClient` con tres shapes (dirección 09-14 → ficha 09-09 → original, cayendo por 42703); `create` **exige la dirección COMPLETA en la fila local ANTES de buscar/crear** (400 `address_required` con `missing[]` — un bundle viejo o un llamador ajeno no la saltea) y, después del create, la ficha y **del link** (así el audit `sync_client_address` lleva el `sellercloud_id`; si el link falla, la dirección queda local y al vincular a mano la fila muestra "Enviar dirección"), `pushAddress` = `setCustomerAddress` + `mark_client_sc_address`; su resultado viaja APARTE en la respuesta (`address_status: 'ok' | 'failed'`, `sc_address_id`, `address_error`), no dentro de `warning`, porque sin dirección el customer no sirve para órdenes; nunca lanza (el customer ya existe y está vinculado). `update` la manda también si hay algo cargado (una a medias es error; sin nada, `address_status: 'skipped'`) y el 502 de "nada entró" considera el fallo de la dirección. Eventos nuevos en `system_logs` (source `sellercloud_customers`): `address_ok` (info), `address_failed` (error), `address_annotate_failed` (warning: entró allá pero no se pudo anotar acá — migración sin correr, por ejemplo). **Edge Function `sellercloud-push-order`**: el 502 del push devuelve además `code` cuando el error lo trae (`customer_no_address`); nada más cambia. **Frontend**: `src/pages/admin/address.jsx` (`ADDRESS_KEYS`/`ADDRESS_REQUIRED`, `addressPayload`/`addressForm`/`addressChanged`/`addressMissing`/`addressIsEmpty`/`addressIsComplete`/`addressSummary`/`addressFieldLabels` y el bloque `AddressFields` — seis inputs con `data-testid="address-*"`, `required` opcional, `<datalist>` de estados de US y Venezuela, `<select>` de país, `idPrefix` para dos bloques en pantalla) y `src/utils/countries.js` (24 países es/en, `US_STATES`, `VE_STATES`, `statesFor`, `countryName`, `defaultCountryForList`: `ve_*` → VE, el resto → US). **`ClientsAdmin.jsx`**: fieldset 📍 Dirección en el alta (`new-client-address`; obligatoria y `required` solo con "Crear también en SellerCloud" prendido; apagado puede ir vacía o completa, nunca a medias — `addressIncomplete` lista lo que falta en el idioma del panel), país propuesto al elegir la lista, va en el mismo insert, `client_created` lleva `address` (resumen) en el contexto; fila de edición con el bloque (`saveAddress` = RPC solo si cambió, ANTES de la ficha — si la RPC la rechaza no se toca la ficha; si el cliente está vinculado, la dirección viaja en el MISMO `update` de la ficha o, si la ficha no cambió, sola con `pushAddressSc`); el form "Crear en SellerCloud" del panel muestra el bloque (guardado → país por la lista) y guarda la dirección por RPC ANTES del create; **badge bajo el ID de SellerCloud** (y columnas Dirección + Dir. SellerCloud en la vista completa: `colCount` 5 / 14): **✓ + la dirección abreviada** — calle, ciudad, estado y código postal, país, sin la segunda línea (`addressShort`), truncada con puntos suspensivos a 18rem; la completa y el #id de la dirección en el tooltip (`address-badge-synced`; ajuste del mismo día a pedido del usuario: "no debería ser un texto que diga literalmente 'Dirección en SellerCloud', directamente se debería poner la dirección abreviada debajo del sellercloud id" — la clave i18n `addressScSynced` se retiró), ⏳ "Dirección sin sincronizar" + **Enviar dirección** (`pending`), ⚠️ "Sin dirección en SellerCloud" + **Reintentar dirección** con el motivo en el tooltip (`failed`), 📍 "Sin dirección cargada" (`none`); tras un create con `address_status: 'failed'` el panel muestra un cuadro ROJO aparte del ✓ (`sc-address-error`) con Reintentar. **`OrdersAdmin.jsx`**: cuando el push rebota por cliente sin dirección (por `code` en vivo o por el texto guardado en `sellercloud_error` de un intento anterior — así se rescatan los 6 de septiembre sin reenviar a ciegas), la fila muestra **📍 Cargar dirección y reenviar** (`push-no-address-cta`) → modal (`push-address-modal`) que lee la dirección local del cliente (con `price_lists(code)` para proponer el país) y al confirmar: RPC `update_client_address` → `update` de `sellercloud-customers` (PUT + relectura) → reenvía la orden; cada paso que falla deja el modal abierto con el motivo (`push-address-error`); sin vínculo avisa que primero hay que vincular. **`AuditLogAdmin.jsx`**: acciones `update_client_address` ("Dirección del cliente", antes → después solo de lo cambiado) y `sync_client_address` ("Dirección enviada a SellerCloud": SC #id · dirección · #id de la dirección), un filtro para las dos. i18n `address*` / `pushNoAddress*` / `actionUpdateClientAddress` / `actionSyncClientAddress` es/en (**621/621**). **Verificado de verdad**: (a) la migración contra PG 18 desechable, corrida DOS veces (idempotente) + 8 bloques de assert — columnas y check ISO-2; admin carga y audita con from/to; no-op sin auditoría; incompleta y país largo rechazados con el mensaje exacto; vendedora la suya sí / la ajena no; `mark_client_sc_address` éxito/fallo/reintento; no se borra una que ya está en SellerCloud pero sí se corrige dejando pendiente, y sin sync previo vaciar sí; `sync_upsert_clients` con update/insert/sin estado/sin zip/país raro/sin claves, payload posterior sin dirección no borra, adopción por teléfono con dirección; grants y firmas únicas — más el **preflight negativo** (sin `business_name` corta con "faltan piezas previas" y no crea ninguna columna); (b) `tests/sc-customers-tests.mjs` 53 → **78** (DTO, body con existentes / en su lugar / por id, secuencia GET → PUT → GET, ID devuelto, reintento sin duplicar, las ajenas sobreviven, la nuestra primera, incompleta corta antes de tocar la API, PUT rechazado con contexto, PUT "ok" sin efecto → error por relectura); `sc-push-tests` sigue en verde; (c) **78/78 Playwright** contra el build real con Supabase y las dos funciones interceptadas (badges de los cuatro estados, el sincronizado con ✓ + la dirección abreviada sin la segunda línea y el #id en el tooltip; Enviar y Reintentar con éxito y con fallo; alta con dirección obligatoria y `required` del navegador; país por lista us/ve; insert con las seis columnas; create + ✓ en la fila nueva; create con dirección rechazada → cuadro rojo + Reintentar desde el panel; Crear desde la fila pide la dirección y la guarda por RPC antes del create; edición con zip nuevo → RPC + `update` solo, sin la RPC de la ficha; a medias → mensaje con exactamente lo que falta y sin RPC; Pedidos: CTA solo en el rechazado, modal con el país, incompleta sin RPC, RPC → update → push en ese orden y la fila con la orden, rechazo en vivo por `code`; inglés; sin errores de página); (d) build limpio. `schema.sql` NO se tocó. **Pendiente (lo hace el usuario)**: (1) correr la migración (`npx supabase db query --linked -f supabase/migration-2026-09-14-client-address.sql` desde `zimaxx-store/`); (2) redesplegar `sellercloud-customers` (→ v8) y `sellercloud-push-order` (→ v15, por el `code`) — **antes, los dos flags del alta en `false`** salvo que se esté reactivando el alta a conciencia, y volver a `true` en `dev` después (regla del punto 88); (3) commit en `dev`, `main` fast-forward, push. Mientras el alta siga bloqueada en producción, lo que se usa de esta tanda es Editar → Dirección + "Enviar dirección" para los clientes ya vinculados y "Cargar dirección y reenviar" en Pedidos para los rechazados. (4) n8n: agregar las claves de dirección al payload de `sync_upsert_clients` para que los customers existentes lleguen con su dirección. **Cómo se retomó tras el corte**: el scratchpad de la sesión cortada (`%TEMP%\claude\<proyecto>\<sesión>\scratchpad`) conservaba `pgtest/` (stubs + asserts ya escritos, el cluster había llegado a levantar) y `pw/address.mjs` (la suite, escrita a las 09:50 y nunca corrida); el `git status` mostraba el código completo. Se re-corrió todo desde cero y pasó sin tocar una línea; el único cambio de código de la retoma fue mover `pushAddress` detrás del `link` en `create`.

90. **Pedido mínimo por lista de precio: $800 en el catálogo de $800, $2,000 en el resto, y el servidor lo hace cumplir** (2026-09-15, a pedido del usuario: "sería bueno limitar la creación de órdenes menores a 2000 al catálogo de 800$"). **Punto de partida**: el mínimo era un $800 plano para TODAS las listas y vivía solo en el navegador (`MIN_ORDER = VITE_MIN_ORDER ?? 800` en `CartDrawer.jsx`); `create_order` no chequeaba monto. El README lo tenía en el roadmap como "enforcement estricto por nivel (mínimo $2,000 para wholesale)". **Impacto medido en producción ANTES de tocar nada** (`npx supabase db query --linked`, solo lectura; pedidos reales no cancelados desde el primero, 2026-07-27): `us_wholesale` 489 pedidos, **244 por debajo de $2,000** (la mediana es exactamente $2,000; 141 directos del catálogo entre $800 y $1,999; 47 por debajo de $800, de los cuales 38 son cotizaciones convertidas desde el panel, 8 directos y 1 manual); `ve_wholesale` 69 / 35 bajo $2,000; `special` 114 / 46; `luzmar` (label "Lista VIP" en producción) 24 / 8; `us_min` 80 / 68 (su mínimo sigue en 800, no le cambia nada); `ve_min` 16 / 16. O sea: **la mitad de los pedidos mayoristas de hoy quedaría bloqueada en el carrito** con la regla nueva — es exactamente lo que se pidió, pero hay que saberlo antes de correr la migración y avisar a las vendedoras. **Interpretación adoptada**: la frase admite otra lectura (re-cotizar al precio de la lista de $800 los pedidos chicos de un mayorista, la "opción C" del roadmap); se eligió **bloquear**, que es lo que dice "limitar la creación" y continúa el aviso "Te faltan $X" que ya existía; la alternativa queda anotada en el roadmap. **Diseño — el dato vive en la base, no en cada llamador** (mismo criterio que el trigger de stock, punto 46): `price_lists.min_order numeric(12,2)`, null = sin mínimo, **default 800** para listas nuevas (`sa_create_price_list` no lo pide y ninguna lista debe nacer sin mínimo por olvido), check `>= 0`. **Semilla SOLO la primera vez que corre** (bloque `DO` guardado por "la columna no existía", así un ajuste a mano sobrevive a re-correr el archivo — probado): `us_min`/`ve_min` 800, `us_wholesale`/`ve_wholesale`/`special`/`luzmar` 2000, `quote` null, cualquier otra 800. `special` y `luzmar` en 2000 por la regla literal del pedido (no son el catálogo de $800); si el negocio quiere otra cosa para alguna es `update public.price_lists set min_order = 800 where code = 'luzmar';`. **`get_catalog`** (copia del de `migration-2026-08-20-client-favorites.sql`, verificado idéntico al vivo con `pg_get_functiondef` vía `db query --linked` antes de copiarlo — y el preflight de la migración corta si el vivo no tiene `is_fav`) suma `client.min_order`. **`create_order`** (copia del de `migration-2026-08-06-require-price.sql`, ídem verificado) rechaza para `kind = 'order'` si el total RECALCULADO < `min_order`: fila en `order_failures` con motivo `total $1,900.00 por debajo del pedido mínimo de $2,000.00 de la lista US Wholesale` + payload + kind + line_count, colocado DESPUÉS del rechazo por producto sin precio (ese tiene prioridad: es más útil para la asesora y un total con líneas sin precio no es comparable) y ANTES del insert; borde inclusivo; las cotizaciones (PDF del carrito, lista `quote`) no tienen mínimo; `p_total` del navegador sigue ignorado; `create_manual_order`/`convert_quote_to_order`/`update_order_items` NO lo chequean (ahí decide una persona con el pedido a la vista — y es justamente por donde entran hoy los pedidos chicos legítimos). Borde asumido: los precios cambian dos veces al día y el servidor recalcula, así que un carrito visto en $2,010 puede recalcularse en $1,990 y rechazarse con el botón habilitado — queda en `order_failures` con el motivo, no se pierde (mismo trato que el rechazo por sin precio). **Frontend**: `src/utils/minOrder.js` nuevo (`minOrderFor(client)`: número → ese; `null` → sin mínimo; clave ausente → `DEFAULT_MIN_ORDER = VITE_MIN_ORDER ?? 800`, que pasó a ser solo el fallback para una base sin migrar); `CartDrawer.jsx` usa `client.min_order` para `belowMin` y el aviso (`data-testid="min-order-notice"` y `"checkout-button"` nuevos); copy `minOrderIs` → "El pedido mínimo de tu lista es" / "The minimum order for your price list is" (sin claves nuevas). El PDF sigue habilitado por debajo del mínimo (es cotización). **`schema.sql` SÍ se tocó esta vez** (columna con default + check + la semilla con `min_order`), porque es una columna nueva de tabla y un install nuevo tiene que nacer con ella; las funciones siguen en su versión vieja como el resto de lo posterior al 08-18. **Verificado de verdad**: (a) la migración contra un **PostgreSQL 18 desechable** con el estado vivo RECONSTRUIDO (3 stubs + `schema.sql` + 07-14 product-upc + 08-14 catalog-upc + 08-20 top-sellers/top-by-line/client-favorites, con lo que el `get_catalog` previo ya tenía `is_fav` como el de producción), corrida **dos veces** con `luzmar = 800` a mano entre medio (la segunda no lo pisó) + **12 bloques de assert**: semilla/default/check; `client.min_order` en las tres ramas (800 / 2000 / null, con `is_fav`/`is_top_line` intactos); 1900 en wholesale rechazado con el motivo EXACTO, payload, kind, line_count y token_hint, sin pedido creado; 2000 entra (inclusivo) y el mismo `request_id` devuelve el mismo pedido sin duplicar; us_min 800 entra / 700 no; PDF (`p_kind = 'quote'`) de $100 y lista `quote` entran sin failure; special 1000 no / 2000 sí; producto sin precio rechaza por "sin precio" y no por mínimo; `p_total` 99999 con total real 500 se rechaza (el motivo lleva el 500); token inválido y payload vacío intactos; todo como rol `anon` (y `anon` sigue sin poder leer `price_lists` directo); marcadores en `pg_get_functiondef` para el sondeo de producción. Gotcha de la prueba: dentro de un `DO` todas las filas comparten `created_at` (una sola transacción), así que "el último `order_failures`" es ambiguo — buscar cada fallo por su motivo. Cluster borrado al terminar. (b) **28/28 Playwright** contra el build real con Supabase interceptado (wholesale 2000: aviso con "$2000.00" y "Te faltan $1500.00", botón de WhatsApp deshabilitado, Descargar PDF habilitado, con $1000 faltan $1000, en $2000 exacto desaparece el aviso y habilita, confirmar → `create_order` UNA vez con `p_kind = 'order'`, `p_total = 2000`, qty 4 → acuse "Pedido registrado"; clave ausente → 800 y "Te faltan $300.00"; us_min 800 explícito; lista quote sin aviso ni bloqueo; inglés "The minimum order for your price list is $2000.00 · You need $1500.00"; sin errores de página). (c) Build limpio. Harness en el scratchpad de la sesión (`pgtest/run-pg.ps1` + `stubs-auth.sql` + `run-all.sh` + `data.sql` + `asserts.sql`; `pw/min-order.mjs` + `run.mjs`), no en el repo. **Cómo terminó el mismo día**: el usuario corrió la migración PRIMERO (`npx supabase db query --linked -f supabase/migration-2026-09-15-price-list-min-order.sql` desde `zimaxx-store/`, verificado en el acto: semilla 800/2000/null, default 800, check, `create_order` con "pedido mínimo", `get_catalog` con `min_order`, grants) con el frontend todavía sin desplegar — el orden inverso al recomendado. Consecuencia mientras dure: el carrito viejo (mínimo $800 plano) deja mandar pedidos mayoristas de $800–$1,999, el servidor los rechaza a `order_failures` con el motivo "pedido mínimo", WhatsApp se abre igual y el cliente ve el aviso de "rechazado"; nada se pierde, pero hay que revisar ese cuadro. **Pendiente (lo hace el usuario)**: (1) deploy del frontend cuanto antes (commit en `dev`, `main` fast-forward, push; con los flags del alta en `false`); (2) revisar en Pedidos los rechazos por "pedido mínimo" que hayan caído en la ventana. (3) Avisar a las vendedoras: sus clientes mayoristas y Special van a ver "El pedido mínimo de tu lista es $2000.00". OJO: el árbol de trabajo de `dev` ya traía sin commitear la dirección del 09-14 (punto 89) con los flags del alta en `true`; antes de cualquier deploy desde `dev`, esos dos flags a `false` (regla del punto 88).

91. **Aviso a la vendedora cuando le recuperan un pedido + registro de cuándo y quién, todo identificado en el Registro de movimientos** (2026-09-17, a pedido del usuario: "enviarle una notificación a la respectiva vendedora cuando se les recupere una orden, para que les salga desde su vista… y así además se puede llevar un registro de cuándo se hizo esa recuperación"; segunda vuelta: "cambia el registro de los pedidos, eso debe estar en registro de movimientos, identificado como tal"). **Punto de partida**: "Recuperar" marcaba `order_failures.recovered_order_id` y nada más — la cotización entraba a la bandeja sin marca, la vendedora dueña no se enteraba, y el único rastro de quién/cuándo estaba en `admin_audit_log`, que ella no puede leer. **Decisiones del usuario** (AskUserQuestion): campanita 🔔 en el header (no badge en la pestaña Pedidos), visto **explícito** con botón (no al abrir), **sin autoaviso** (si la recupera la dueña nace vista). **Base** (`migration-2026-09-17-recovery-notifications.sql`): `order_failures` + `recovered_at` / `recovered_by` / `recovered_by_email` / `recovery_seen_at` (null = pendiente) con backfill de las históricas desde `admin_audit_log` (por `order_id`, fallback `detail->>'failure_id'` como texto, fallback `orders.created_at`), todas vistas para no inundar; `recover_order_failure` las llena (`recovery_seen_at = now()` si `v_client.vendedora_id is not distinct from current_vendedora_id()`), pasa a compare-and-set y suma al detail `vendedora_id`/`notify_vendedora`; RPC `mark_recoveries_seen(uuid[]) → integer` que solo alcanza filas recuperadas, pendientes y de SUS clientes (admin → 0; tope 500) y audita cada visto como `recovery_seen` en la misma sentencia. Sin RLS nueva. **Hallazgo al correrla**: el preflight abortó porque el `recover_order_failure` vivo era el del **08-05** — la 08-13 "siempre cotización" NUNCA corrió (el doc la daba por corrida desde el 08-19); las 89 recuperaciones del 08-07 al 09-17 crearon pedidos REALES (34 atendidos, 50 cancelados, 5 nuevos). La 09-17 pasó a aceptar las dos versiones conocidas y ABSORBE a la 08-13 (que ganó un guard); el usuario aceptó el cambio de comportamiento. **Frontend**: `useRecoveryNotices.js` (estado único en `AdminLayout`, poll 60 s + `visibilitychange`, apagado para admin, `unavailable` con 42703/PGRST202), `RecoveryBell.jsx` (contador, "Se recuperó el pedido / la cotización de X", cuándo y por quién, **Ver** → `/admin/orders?recovered=<order_id>`, **Marcar visto**, **Marcar todos**), chip ♻️ Recuperado por fila en `OrdersAdmin.jsx` (tooltip cuándo y quién; ventana sobre `recovered_at`), deep link `?recovered=` atendido al terminar la carga ("Ver" limpia filtros, despliega `tr#order-<id>` y hace scroll, ampliando a todo el historial si hace falta). **Hubo un cuadro verde "♻️ Pedidos recuperados" en Pedidos (registro con "Nuevo"/"Marcar visto") y el usuario pidió quitarlo el mismo día** ("quiero que el registro quede nada más en el registro de movimientos… el panel verde que se ve en pedidos quitalo de ahí, el rojo… ese sí lo puedes dejar"): el registro vive SOLO en el Registro de movimientos; el visto se marca desde la campanita. `fmtDateTime`/`fmtDay`/`agoLabel` se movieron a `utils/format.js`. **Registro de movimientos** (`AuditLogAdmin.jsx`): etiqueta por `detail` — "Pedido recuperado como cotización" / "Cotización recuperada" / "Pedido recuperado como pedido real" (las 89 viejas) — con intento original, motivo, líneas, total y aviso pendiente; acción nueva `recovery_seen` ("Aviso de recuperación visto"); y `dismiss_order_failure` por fin con etiqueta ("Intento descartado": caía en reasignación). **Verificación**: PG 18 desechable con el cuerpo vivo bajado de producción (`pg_get_functiondef`) — migración ×2, guard de la 08-13, camino 08-13, 11 bloques de assert (backfill, admin → pendiente, dueña → vista, ajena → rechazada, sin vendedora → vista, admin+vendedora, doble recuperación, mark por dueña/ajena/admin/sin rol/anon/tope + auditoría, RLS, grants, idempotencia), 2 preflights negativos; build limpio; Playwright 49/49 (vendedora, admin + Registro, base sin migración). **Estado al cierre**: migración SIN correr en producción (primer intento abortado a propósito), frontend SIN desplegar; orden migración → frontend; `dev` con `SC_CREATE_ENABLED = true` → a `false` antes del push a `main`.
