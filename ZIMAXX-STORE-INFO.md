...
>
> Creado: 2026-07-02. Última actualización: 2026-09-09, **hotfix: alta de
> clientes en SellerCloud DESACTIVADA** (punto 78, rama
> `hotfix/desactivar-creacion-sc`, a pedido del usuario — la creación de
> customers se repara en otra rama): flag `SC_CREATE_ENABLED = false` en
> `ClientsAdmin.jsx` (toggle "Crear también en SellerCloud" apagado y
> deshabilitado con aviso; el panel SellerCloud solo busca y vincula, sin
> "Crear en SellerCloud"/"Crear de todos modos") + candado `CREATE_ENABLED =
> false` en la Edge Function `sellercloud-customers` (`action:'create'` → 503
> "temporalmente desactivada"; search/link siguen). **El candado del servidor
> recién vale tras `npx supabase functions deploy sellercloud-customers`**;
> para reactivar, los dos flags en `true` + deploy de frontend y función.
>
...
78. **Hotfix: alta de clientes en SellerCloud DESACTIVADA** (2026-09-09, rama `hotfix/desactivar-creacion-sc`, a pedido del usuario: "desactiva la función de crear clientes en SellerCloud, la estamos reparando en otra rama"). Se apaga **por flag, sin borrar código**, para que la rama de reparación no tenga que reconstruir nada y reactivar sea cambiar dos constantes. **Frontend** (`ClientsAdmin.jsx`): `SC_CREATE_ENABLED = false` — `EMPTY_CLIENT.scCreate` nace en false, el toggle "Crear también en SellerCloud" se muestra **apagado y deshabilitado** (cursor not-allowed, tooltip) con el aviso "El alta en SellerCloud está temporalmente desactivada." en lugar del hint (i18n `scCreateDisabled`, es/en), el nombre vuelve a pedirse ENTERO (sin nombre/apellido partidos), `createClient` ignora el toggle aunque algo lo prenda (`scOn = SC_CREATE_ENABLED && …`), el panel SellerCloud en estado `candidates` ya no dibuja "Crear en SellerCloud"/"Crear de todos modos" (muestra el aviso; con candidatos el texto pasa a "Elegí el correcto para vincularlo." — `scPickToLinkOnly` — sin el "o crealo si no es ninguno"), y `createSc` corta al principio con `status:'failed'` + aviso sin llamar a la Edge Function. **"🔍 Buscar en SellerCloud" → candidatos → "Vincular" sigue funcionando igual.** **Edge Function `sellercloud-customers`**: candado espejo `CREATE_ENABLED = false` → `action:'create'` responde **503** `{error: 'El alta de clientes en SellerCloud está temporalmente desactivada.'}` antes de leer nada (un bundle viejo abierto en un navegador seguiría mandando create; `search`/`link` intactos). **Para reactivar**: los dos flags en `true` + deploy del frontend y `npx supabase functions deploy sellercloud-customers`. Docs: fila Clientes del README (que además no documentaba el vínculo con SellerCloud del punto 75). **Verificado**: build de Vite limpio; `sc-customers-tests` 26/26 (no toca `index.ts`, sigue igual); **37 aserciones Playwright** contra el build real con Supabase interceptado (5 bloques: alta con toggle apagado+deshabilitado, un solo campo Nombre, click forzado no lo prende, insert local con name entero y CERO llamadas a la Edge Function, sin panel tras el alta; fila sin vínculo → buscar sin candidatos → aviso y ningún botón de crear; con candidatos → texto solo-vincular, "Vincular" llama `link` con el ID elegido y la fila muestra el ID, ninguna `create`; vendedora con el mismo candado; aviso traducido en inglés; sin errores de página en ningún bloque). **Pendiente**: merge a `main`, deploy del frontend (Netlify) y redeploy de la Edge Function para que el candado del servidor esté vivo. Nada corrió contra producción.
